...
describe("allure agent-mode guidance", () => {
  it("should keep stable guidance in the package README and generated run playbook", async () => {
    ...
    await expectTextToContainAll("generated AGENTS guide", agentsGuide, [
      ...
      "allure agent latest",
      "a user asks for the human-readable report from the last run",
    ...
    await expectTextToContainAll("plugin-agent README", readme, [
      ...
      "Check `manifest/human-report.json`, the Human",
      "Report section in `index.md`, or `allure agent query --latest summary`",
      "If you need the human-readable report from the most recent agent run",
