...
import type { AttributeSelectorOperator, AttributeSelectorPart } from "@/utils/selectorParser";
...
function queryRole(scope: SelectorRoot, options: RoleEngineOptions, internal: boolean): Element[] {
  ...
  const match = (element: Element) => {
    ...
    if (options.checked !== undefined && getAriaChecked(element) !== options.checked) return;
