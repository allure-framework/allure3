...
export class UtilityScript {
  constructor(isUnderTest: boolean) {
    if (isUnderTest) this._setBuiltins();
  }
  ...
  evaluate(
    ...
    const parameters = [];
    for (let i = 0; i < args.length; i++) parameters[i] = this.parseEvaluationResultValue(args[i], handles);
  ...
  private _setBuiltins() {
    ...
    window.builtinSetTimeout = (callback: Function, timeout: number) => {
      if (window.__pwClock?.builtin) return window.__pwClock.builtin.setTimeout(callback, timeout);
      return setTimeout(callback, timeout);
    };

    window.builtinClearTimeout = (id: number) => {
      if (window.__pwClock?.builtin) return window.__pwClock.builtin.clearTimeout(id);
      return clearTimeout(id);
    };

    window.builtinSetInterval = (callback: Function, timeout: number) => {
      if (window.__pwClock?.builtin) return window.__pwClock.builtin.setInterval(callback, timeout);
      return setInterval(callback, timeout);
    };

    window.builtinClearInterval = (id: number) => {
      if (window.__pwClock?.builtin) return window.__pwClock.builtin.clearInterval(id);
      return clearInterval(id);
    };

    window.builtinRequestAnimationFrame = (callback: FrameRequestCallback) => {
      if (window.__pwClock?.builtin) return window.__pwClock.builtin.requestAnimationFrame(callback);
      return requestAnimationFrame(callback);
    };

    window.builtinCancelAnimationFrame = (id: number) => {
      if (window.__pwClock?.builtin) return window.__pwClock.builtin.cancelAnimationFrame(id);
