 ...
 */
import type { AttributeSelectorPart } from "@/utils/selectorParser";
      ...
      if (root instanceof HTMLInputElement && (root.type === "submit" || root.type === "button")) {
        ...
        for (let child = root.firstChild; child; child = child.nextSibling) {
          if (child.nodeType === Node.TEXT_NODE) {
            ...
            currentImmediate = "";
            if (child.nodeType === Node.ELEMENT_NODE) value.full += elementText(cache, child as Element).full;
          }
        }
        if (currentImmediate) value.immediate.push(currentImmediate);
        if ((root as Element).shadowRoot) value.full += elementText(cache, (root as Element).shadowRoot!).full;
...
export function elementMatchesText(
  ...
  for (let child = element.firstChild; child; child = child.nextSibling) {
    if (child.nodeType === Node.ELEMENT_NODE && matcher(elementText(cache, child as Element))) return "selfAndChildren";
  }
