 ...
 */
import type * as channels from "@/types/channels";
import { parseAriaSnapshot } from "@/utils/ariaSnapshot";
import type { AriaTemplateNode } from "@/utils/ariaSnapshot";
import type { CSSComplexSelectorList } from "@/utils/cssParser";
import { asLocator } from "@/utils/locatorGenerators";
import type { Language } from "@/utils/locatorGenerators";
import {
  parseAttributeSelector,
  parseSelector,
  stringifySelector,
  visitAllSelectorParts,
} from "@/utils/selectorParser";
import type { NestedSelectorBody, ParsedSelector, ParsedSelectorPart } from "@/utils/selectorParser";
import { cacheNormalizedWhitespaces, normalizeWhiteSpace, trimStringWithEllipsis } from "@/utils/string";
import { generateAriaTree, getAllByAria, matchesAriaTree, renderAriaTree } from "./ariaSnapshot.js";
import type { AriaNode, AriaSnapshot } from "./ariaSnapshot.js";
import {
...
import { ReactEngine } from "./reactSelectorEngine.js";
...
import { VueEngine } from "./vueSelectorEngine.js";
...
export class InjectedScript {
  ...
  constructor(
    ...
    if (isUnderTest) (this.window as any).__injectedScript = this;
  }

  builtinSetTimeout(callback: Function, timeout: number) {
    if (this.window.__pwClock?.builtin) return this.window.__pwClock.builtin.setTimeout(callback, timeout);
    return this.window.setTimeout(callback, timeout);
  }

  builtinClearTimeout(timeout: number | undefined) {
    if (this.window.__pwClock?.builtin) return this.window.__pwClock.builtin.clearTimeout(timeout);
    return this.window.clearTimeout(timeout);
  }

  builtinRequestAnimationFrame(callback: FrameRequestCallback) {
    if (this.window.__pwClock?.builtin) return this.window.__pwClock.builtin.requestAnimationFrame(callback);
  ...
  ariaSnapshot(node: Node, options?: { mode?: "raw" | "regex"; id?: boolean }): string {
    ...
    return renderAriaTree(ariaSnapshot.root, {
      ...options,
      ids: options?.id ? ariaSnapshot.ids : undefined,
    });
  }
  ...
  querySelectorAll(selector: ParsedSelector, root: Node): Element[] {
    ...
    if (!(root as any)["querySelectorAll"]) throw this.createStacklessError("Node is not queryable.");
    ...
    return {
      queryAll: (root: SelectorRoot, selector: string): Element[] => {
        return this._evaluator.query(
          {
            scope: root as Document | Element,
            pierceShadow: shadow,
          },
          toCSS(selector),
        );
  ...
  describeIFrameStyle(iframe: Element):
    | "error:notconnected"
    | "transformed"
    | {
        left: number;
  ...
  private async _checkElementIsStable(node: Node): Promise<"error:notconnected" | boolean> {
    ...
    const raf = () => {
      try {
        ...
        else this.builtinRequestAnimationFrame(raf);
      } catch (e) {
        reject(e);
      }
    };
    ...
    for (let index = 0; index < options.length; index++) {
      ...
      const filter = (
        ...
        if (optionToSelect.valueOrLabel !== undefined)
          matches =
            matches && (optionToSelect.valueOrLabel === option.value || optionToSelect.valueOrLabel === option.label);
  ...
  expectHitTarget(hitPoint: { x: number; y: number }, targetElement: Element) {
    ...
    for (let index = roots.length - 1; index >= 0; index--) {
      ...
      if (index && innerElement !== (roots[index - 1] as ShadowRoot).host) break;
    }
    ...
    switch (eventType.get(type)) {
      ...
      case "devicemotion":
        try {
          ...
          const { bubbles, cancelable, acceleration, accelerationIncludingGravity, rotationRate, interval } =
            eventInit as {
              bubbles: boolean;
              cancelable: boolean;
              acceleration: DeviceMotionEventAcceleration;
  ...
  markTargetElements(markedElements: Set<Element>, callId: string) {
    if (this._markedElements?.callId !== callId) this._markedElements = undefined;
  ...
  private async expectSingleElement(
...

    {
      ...
      if (result) {
        if (result.received === "error:notconnected") throw this.createStacklessError("Element is not connected");
        return result;
      }
    }
  ...
  private expectArray(
    ...
    else if (expression === "to.have.class.array") received = elements.map((e) => e.classList.toString());
...
const eventType = new Map<
  string,
  "mouse" | "keyboard" | "touch" | "pointer" | "focus" | "drag" | "wheel" | "deviceorientation" | "devicemotion"
>([
  ["auxclick", "mouse"],
  ["click", "mouse"],
  ["dblclick", "mouse"],
