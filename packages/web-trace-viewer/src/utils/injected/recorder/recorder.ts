...
class InspectTool implements RecorderTool {
  ...
  onContextMenu(event: MouseEvent) {
    if (
      ...
      this._hoveredModel.tooltipList = selectors.map((selector) =>
        this._recorder.injectedScript.utils.asLocator(this._recorder.state.language, selector),
      );
      this._hoveredModel.tooltipListItemSelected = (index: number | undefined) => {
        if (index === undefined) this._reset(true);
        else this._commit(selectors[index], hoveredModel);
      };
  ...
  onKeyDown(event: KeyboardEvent) {
    consumeEvent(event);
    if (event.key === "Escape") {
      if (this._hoveredModel?.tooltipListItemSelected) this._reset(true);
      else if (this._assertVisibility) this._recorder.setMode("recording");
    }
  }
...
class RecordActionTool implements RecorderTool {
  ...
  private _cancelPendingClickAction() {
    if (this._pendingClickAction) clearTimeout(this._pendingClickAction.timeout);
  ...
  private _shouldIgnoreMouseEvent(event: MouseEvent): boolean {
    ...
    if (nodeName === "INPUT" && ["date", "range"].includes((target as HTMLInputElement).type)) return true;
    return false;
  }
  ...
  private _consumedDueWrongTarget(event: Event): boolean {
    if (this._activeModel && this._activeModel.elements[0] === this._recorder.deepEventTarget(event)) return false;
    consumeEvent(event);
    return true;
  }
  ...
  private _shouldGenerateKeyPressFor(event: KeyboardEvent): boolean {
    ...
    if (event.key.length === 1 && !hasModifier) return !!asCheckbox(this._recorder.deepEventTarget(event));
    return true;
  }
...
class TextAssertionTool implements RecorderTool {
  ...
  onMouseMove(event: MouseEvent) {
    ...
    if (this._kind === "text" || this._kind === "snapshot")
      this._hoverHighlight = this._recorder.injectedScript.utils.elementText(this._textCache, target).full
        ? { elements: [target], selector: "" }
        : null;
    else
      this._hoverHighlight = this._elementHasValue(target)
        ? this._recorder.injectedScript.generateSelector(target, {
            testIdAttributeName: this._recorder.state.testIdAttributeName,
          })
        : null;
    if (this._hoverHighlight) this._hoverHighlight.color = "#8acae480";
  ...
  private _generateAction(): actions.AssertAction | null {
    ...
    if (this._kind === "value") {
      ...
      this._hoverHighlight = this._recorder.injectedScript.generateSelector(target, {
        testIdAttributeName: this._recorder.state.testIdAttributeName,
        forTextExpect: true,
      });
  ...
  private _renderValue(action: actions.Action) {
    if (action?.name === "assertText") return this._recorder.injectedScript.utils.normalizeWhiteSpace(action.text);
    if (action?.name === "assertChecked") return String(action.checked);
    if (action?.name === "assertValue") return action.value;
    if (action?.name === "assertSnapshot") return action.snapshot;
    return "";
  }
...
export class Recorder {
  ...
  setUIState(state: UIState, delegate: RecorderDelegate) {
    ...
    if (
      ...
      if (state.actionPoint) this.highlight.showActionPoint(state.actionPoint.x, state.actionPoint.y);
    ...
    if (state.actionSelector !== this._lastHighlightedSelector) {
      const model = state.actionSelector
        ? querySelector(this.injectedScript, state.actionSelector, this.document)
        : null;
    ...
    else if (highlight !== "noop") this._updateHighlight(highlight, false);
  }
