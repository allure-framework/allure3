...
import {
  ...
  getByTextSelector,
  getByTitleSelector,
} from "@/utils/locatorUtils";
import type { ByRoleOptions } from "@/utils/locatorUtils";
import { escapeForTextSelector } from "@/utils/string";
...
class Locator {
  ...
  constructor(
    ...
    if (options?.hasText) selector += ` >> internal:has-text=${escapeForTextSelector(options.hasText, false)}`;
    if (options?.hasNotText)
      selector += ` >> internal:has-not-text=${escapeForTextSelector(options.hasNotText, false)}`;
    if (options?.has) selector += ` >> internal:has=` + JSON.stringify(options.has[selectorSymbol]);
    if (options?.hasNot) selector += ` >> internal:has-not=` + JSON.stringify(options.hasNot[selectorSymbol]);
    ...
    ): Locator => self.locator(getByAltTextSelector(text, options));
    self.getByLabel = (
      text: string | RegExp,
      options?: {
        exact?: boolean;
      },
    ): Locator => self.locator(getByLabelSelector(text, options));
    self.getByPlaceholder = (
      text: string | RegExp,
      options?: {
        exact?: boolean;
      },
    ): Locator => self.locator(getByPlaceholderSelector(text, options));
    self.getByText = (
      text: string | RegExp,
      options?: {
        exact?: boolean;
      },
    ): Locator => self.locator(getByTextSelector(text, options));
    self.getByTitle = (
      text: string | RegExp,
      options?: {
        exact?: boolean;
      },
    ): Locator => self.locator(getByTitleSelector(text, options));
