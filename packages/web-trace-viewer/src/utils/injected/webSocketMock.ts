...
export function inject(globalThis: GlobalThis) {
  if ((globalThis as any).__pwWebSocketDispatch) return;
  ...
  class WebSocketMock extends EventTarget {
    ...
    set binaryType(type) {
      ...
      if (this._ws) this._ws.binaryType = type;
    }
    ...
    set onclose(listener) {
      if (this._oncloseListener) this.removeEventListener("close", this._oncloseListener as any);
      this._oncloseListener = listener;
      if (this._oncloseListener) this.addEventListener("close", this._oncloseListener as any);
    }
    ...
    set onmessage(listener) {
      if (this._onmessageListener) this.removeEventListener("message", this._onmessageListener as any);
      this._onmessageListener = listener;
      if (this._onmessageListener) this.addEventListener("message", this._onmessageListener as any);
    }
    ...
    close(code?: number, reason?: string): void {
      ...
      else binding({ type: "onClosePage", id: this._id, code, reason, wasClean: true });
    }
    ...
    _apiSendToServer(message: WebSocketMessage) {
      ...
      if (this._ws.readyState === WebSocketMock.CONNECTING) this._wsBufferedMessages.push(message);
    ...
    _apiConnect() {
      ...
      this._ws.onopen = () => {
        for (const message of this._wsBufferedMessages) this._ws!.send(message);
      ...
      this._ws.onmessage = (event) => {
        if (this._passthrough) this._apiSendToPage(event.data);
        else messageToData(event.data, (data) => binding({ type: "onMessageFromServer", id: this._id, data }));
      };
    ...
    _apiClosePage(code: number | undefined, reason: string | undefined, wasClean: boolean) {
      ...
      else binding({ type: "onClosePage", id: this._id, code, reason, wasClean });
    }
    ...
    private _onWSClose(code: number | undefined, reason: string | undefined, wasClean: boolean) {
      ...
      else binding({ type: "onCloseServer", id: this._id, code, reason, wasClean });
      if (this._ws) {
    ...
    private _maybeCleanup() {
      if (this.readyState === WebSocketMock.CLOSED && !this._ws) idToWebSocket.delete(this._id);
    }
  }
