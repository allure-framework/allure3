...
} from "@/utils/ariaSnapshot";
import { escapeRegExp, longestCommonSubstring, normalizeWhiteSpace } from "@/utils/string";
import { getElementComputedStyle } from "./domUtils.js";
...
export function generateAriaTree(rootElement: Element): AriaSnapshot {
  ...
  const visit = (ariaNode: AriaNode, node: Node) => {
    ...
    if (roleUtils.isElementHiddenForAria(element)) return;
...
function toAriaNode(element: Element): AriaNode | null {
  ...
  if (roleUtils.kAriaCheckedRoles.includes(role)) result.checked = roleUtils.getAriaChecked(element);

  if (roleUtils.kAriaDisabledRoles.includes(role)) result.disabled = roleUtils.getAriaDisabled(element);

  if (roleUtils.kAriaExpandedRoles.includes(role)) result.expanded = roleUtils.getAriaExpanded(element);

  if (roleUtils.kAriaLevelRoles.includes(role)) result.level = roleUtils.getAriaLevel(element);

  if (roleUtils.kAriaPressedRoles.includes(role)) result.pressed = roleUtils.getAriaPressed(element);

  if (roleUtils.kAriaSelectedRoles.includes(role)) result.selected = roleUtils.getAriaSelected(element);

  if (element instanceof HTMLInputElement || element instanceof HTMLTextAreaElement) {
    if (element.type !== "checkbox" && element.type !== "radio") result.children = [element.value];
  }
...
function matchesNode(node: AriaNode | string, template: AriaTemplateNode, depth: number): boolean {
  ...
  if (node !== null && typeof node === "object" && template.kind === "role") {
    ...
    if (!containsList(node.children || [], template.children || [], depth)) return false;
    return true;
  }
  return false;
}
...
function containsList(children: (AriaNode | string)[], template: AriaTemplateNode[], depth: number): boolean {
  ...
  for (const t of tt) {
    let c = cc.shift();
    while (c) {
      if (matchesNode(c, t, depth + 1)) break;
      c = cc.shift();
    }
    if (!c) return false;
  }
