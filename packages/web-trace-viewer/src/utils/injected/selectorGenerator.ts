...
import { closestCrossShadow, isElementVisible, isInsideScope, parentElementOrShadowHost } from "./domUtils.js";
import type { InjectedScript } from "./injectedScript.js";
import { beginAriaCaches, endAriaCaches, getAriaRole, getElementAccessibleName } from "./roleUtils.js";
  ...
  try {
    ...
    if (options.forTextExpect) {
      ...
      if (options.multiple) {
        ...
        const withoutText = generateSelectorFor(injectedScript, targetElement, {
          ...options,
          noText: true,
        });
        ...
        if (withText && hasCSSIdToken(withText)) {
          tokens.push(
            generateSelectorFor(injectedScript, targetElement, {
              ...options,
              noCSSId: true,
            }),
          );
