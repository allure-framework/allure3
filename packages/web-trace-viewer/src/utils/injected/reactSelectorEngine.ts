 ...
 */
import { parseAttributeSelector } from "@/utils/selectorParser";
import { isInsideScope } from "./domUtils.js";
import type { SelectorEngine, SelectorRoot } from "./selectorEngine.js";
import { matchesComponentAttribute } from "./selectorUtils.js";
...
function getChildren(reactElement: ReactVNode): ReactVNode[] {
  ...
  if (reactElement.child) {
    ...
    for (let child: ReactVNode | undefined = reactElement.child; child; child = child.sibling) children.push(child);
    return children;
  }
  ...
  do {
    ...
    if (node instanceof Element && node.hasAttribute("data-reactroot")) {
      for (const key of Object.keys(node)) {
        ...
        if (key.startsWith("__reactInternalInstance") || key.startsWith("__reactFiber")) roots.push((node as any)[key]);
      }
    }
