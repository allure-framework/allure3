...
export function isElementHiddenForAria(element: Element): boolean {
  ...
  if (style?.display === "contents" && !isSlot) {
    ...
    for (let child = element.firstChild; child; child = child.nextSibling) {
      if (child.nodeType === 1 /* Node.ELEMENT_NODE */ && !isElementHiddenForAria(child as Element)) return false;
      if (child.nodeType === 3 /* Node.TEXT_NODE */ && isVisibleTextNode(child as Text)) return false;
    }
    return true;
  }
...
function getPseudoContentImpl(element: Element, pseudoStyle: CSSStyleDeclaration | undefined) {
  ...
  if (!pseudoStyle || pseudoStyle.display === "none" || pseudoStyle.visibility === "hidden") return "";
...
function getChecked(element: Element, allowMixed: boolean): boolean | "mixed" | "error" {
  ...
  // https://www.w3.org/TR/html-aam-1.0/#html-attribute-state-and-property-mappings
  if (allowMixed && tagName === "INPUT" && (element as HTMLInputElement).indeterminate) return "mixed";
...
function belongsToDisabledFieldSet(element: Element | null): boolean {
  ...
  if (elementSafeTagName(element) === "FIELDSET" && element.hasAttribute("disabled")) return true;
