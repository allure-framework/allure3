...
export class TestTree {
  ...
  constructor(
    ...
    const visitSuite = (
      ...
      for (const suite of parentSuite.suites) {
        ...
        let group = parentGroup.children.find((item) => item.kind === "group" && item.title === suite.title) as
          | GroupItem
          | undefined;
        if (!group) {
          group = {
      ...
      for (const test of parentSuite.tests) {
        ...
        if ((result as any)?.[statusEx] === "scheduled") status = "scheduled";
        else if ((result as any)?.[statusEx] === "running") status = "running";
        else if (result?.status === "skipped") status = "skipped";
        else if (result?.status === "interrupted") status = "none";
        else if (result && test.outcome() !== "expected") status = "failed";
        else if (result && test.outcome() === "expected") status = "passed";
  ...
  fileNames(): string[] {
    const result = new Set<string>();
    const visit = (treeItem: TreeItem) => {
      if (treeItem.kind === "group" && treeItem.subKind === "file") result.add(treeItem.id);
...
export function collectTestIds(treeItem: TreeItem): Set<string> {
  ...
  const visit = (treeItem: TreeItem) => {
    if (treeItem.kind === "case") treeItem.tests.map((t) => t.id).forEach((id) => testIds.add(id));
