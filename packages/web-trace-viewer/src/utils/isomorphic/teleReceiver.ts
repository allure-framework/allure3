 ...
 */
import type { Annotation } from "@/types/config";
import type { ReporterV2 } from "@/types/reporterV2";
...
export type JsonConfig = Pick<
  ...
  "configFile" | "globalTimeout" | "maxFailures" | "metadata" | "rootDir" | "version" | "workers"
>;
...
export class TeleReporterReceiver {
  ...
  private _onProject(project: JsonProject) {
    let projectSuite = this._options.mergeProjects
      ? this._rootSuite.suites.find((suite) => suite.project()!.name === project.name)
      : undefined;
    if (!projectSuite) {
...
export function computeTestCaseOutcome(test: reporterTypes.TestCase) {
  ...
  if (expected === 0 && unexpected === 0) {
    return "skipped";
  } // all results were skipped or interrupted
  if (unexpected === 0) {
    return "expected";
  } // no failures, just expected+skipped
