...
import { escapeForAttributeSelector, escapeForTextSelector } from "./string.js";
...
function transform(template: string, params: TemplateParams, testIdAttributeName: string): string {
  ...
  while (true) {
    ...
    template = `${prefix.replace(/=$/, "2=")}$${paramsCountBeforeHas + 1}${shiftParams(
      template.substring(end + extraSymbol),
      paramsCountInHas - 1,
    )}`;
