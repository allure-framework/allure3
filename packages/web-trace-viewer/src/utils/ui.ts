...
export function useAsyncMemo<T>(fn: () => Promise<T>, deps: React.DependencyList, initialValue: T, resetValue?: T) {
  ...
  React.useEffect(() => {
    let canceled = false;
    if (resetValue !== undefined) setValue(resetValue);
    fn().then((value) => {
...
export function useMeasure<T extends Element>() {
  ...
  React.useLayoutEffect(() => {
    ...
    const resizeObserver = new ResizeObserver((entries: any) => {
      ...
      if (entry && entry.contentRect) setMeasure(entry.contentRect);
    });
...
export function bytesToString(bytes: number): string {
  ...
  const kb = bytes / 1024;
  if (kb < 1000) return kb.toFixed(1) + "K";
...
export class Settings {
  ...
  getObject<T>(name: string, defaultValue: T): T {
    if (!localStorage[name]) return defaultValue;
    try {
...
export function scrollIntoViewIfNeeded(element: Element | undefined) {
  if (!element) return;
  if ((element as any)?.scrollIntoViewIfNeeded) (element as any).scrollIntoViewIfNeeded(false);
  else element?.scrollIntoView();
}

const kControlCodesRe = "\\u0000-\\u0020\\u007f-\\u009f";
export const kWebLinkRe = new RegExp(
...
export function useFlash(): [boolean, EffectCallback] {
  ...
  const trigger = React.useCallback<React.EffectCallback>(() => {
    ...
    setFlash((currentlyFlashing) => {
      ...
      if (!currentlyFlashing) return true;
