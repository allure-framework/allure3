...
import { CodeMirrorWrapper } from "@/ui/components/codeMirrorWrapper";
import type { SourceHighlight } from "@/ui/components/codeMirrorWrapper";
...
import type { SourceLocation, SourceModel } from "./modelUtil.js";
import "./sourceTab.css";
import { StackTraceView } from "./stackTrace.js";
...
export const SourceTab: React.FunctionComponent<{
  ...
  const { source, highlight, targetLine, fileName, location } = useAsyncMemo<{
    source: SourceModel;
    targetLine?: number;
    fileName?: string;
    ...
    async () => {
      ...
      if (!source) {
        source = {
          errors: fallbackLocation?.source?.errors || [],
          content: fallbackLocation?.source?.content,
        };
        sources.set(file, source);
      }

      const location = shouldUseFallback ? fallbackLocation! : actionLocation;
      const targetLine = shouldUseFallback
        ? fallbackLocation?.line || source.errors[0]?.line || 0
        : actionLocation.line;
      const fileName = rootDir && file.startsWith(rootDir) ? file.substring(rootDir.length + 1) : file;
      const highlight: SourceHighlight[] = source.errors.map((e) => ({
        type: "error",
        line: e.line,
        message: e.message,
      }));
    ...
    [stack, selectedFrame, rootDir, fallbackLocation],
    {
      source: {
        errors: [],
        content: "Loading\u2026",
        ...
        <div className="vbox" data-testid="source-code">
          {fileName && (
            <Toolbar>
              ...
              {location && (
                <ToolbarButton icon="link-external" title="Open in VS Code" onClick={openExternally}></ToolbarButton>
              )}
            </Toolbar>
          )}
          <CodeMirrorWrapper
