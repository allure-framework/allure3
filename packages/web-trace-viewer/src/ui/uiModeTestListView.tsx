...
import type { TeleSuiteUpdaterTestModel } from "@/utils/isomorphic/teleSuiteUpdater";
import type { TestServerConnection } from "@/utils/isomorphic/testServerConnection.js";
import type { TestTree, TreeItem } from "@/utils/isomorphic/testTree";
  ...
  return (
    <TestTreeView
      ...
      render={(treeItem) => {
        ...
        return (
          <div className="hbox ui-mode-tree-item" aria-labelledby={`${labelId} ${timeId}`}>
            <div id={labelId} className="ui-mode-tree-item-title">
              <span>{treeItem.title}</span>
              {treeItem.kind === "case"
                ? treeItem.tags.map((tag) => (
                    <TagView key={tag} tag={tag.slice(1)} onClick={(e) => handleTagClick(e, tag)} />
                  ))
                : null}
            </div>
            {!!treeItem.duration && treeItem.status !== "skipped" && (
              <div id={timeId} className="ui-mode-tree-item-time">
            ...
            <Toolbar noMinHeight={true} noShadow={true}>
              <ToolbarButton
                icon="play"
                title="Run"
                onClick={() => runTreeItem(treeItem)}
                disabled={!!runningState && !runningState.completed}
              ></ToolbarButton>
              <ToolbarButton
                icon="go-to-file"
                title="Show source"
                onClick={onRevealSource}
                style={treeItem.kind === "group" && treeItem.subKind === "folder" ? { visibility: "hidden" } : {}}
              ></ToolbarButton>
              {!watchAll && (
                <ToolbarButton
                  icon="eye"
...
function itemLocation(
  ...
  model: TeleSuiteUpdaterTestModel | undefined,
  ...
  return {
    ...
    source: {
      errors: model.loadErrors
        .filter((e) => e.location?.file === item.location.file)
        .map((e) => ({
