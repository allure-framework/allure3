...
import type { StackFrame } from "@/types/channels.js";
import type { GitCommitInfo } from "@/types/testIsomorphic.js";
import { ErrorMessage } from "@/ui/components/errorMessage";
import { fixTestPrompt } from "@/ui/components/prompts";
import { ToolbarButton } from "@/ui/components/toolbarButton";
import type { Language } from "@/utils/locatorGenerators";
import { useAsyncMemo } from "@/utils/ui";
import { renderAction } from "./actionList.js";
import { AIConversation } from "./aiConversation.js";
import { attachmentURL } from "./attachmentsTab.js";
import { CopyToClipboardTextButton } from "./copyToClipboard.js";
import { useIsLLMAvailable, useLLMChat } from "./llm.js";
...
export function GitCommitInfoProvider({
  children,
  gitCommitInfo,
...
function usePageSnapshot(actions: modelUtil.ActionTraceEventInContext[]) {
  return useAsyncMemo<string | undefined>(
    async () => {
        ...
        for (const attachment of action.attachments ?? []) {
          if (attachment.name === "pageSnapshot") {
            const response = await fetch(
              attachmentURL({
                ...attachment,
                traceUrl: action.context.traceUrl,
              }),
            );
...
const CopyPromptButton: React.FC<{
  ...
  const prompt = React.useMemo(() => fixTestPrompt(error, diff, pageSnapshot), [error, diff, pageSnapshot]);
  ...
  return (
    <CopyToClipboardTextButton
      ...
      copiedDescription={
        <>
          Copied <span className="codicon codicon-copy" style={{ marginLeft: "5px" }} />
        </>
      }
      ...
      >
        ...
        {location && (
          <div className="action-location">
            @{" "}
            <span title={longLocation} onClick={() => revealInSource(error)}>
              {location}
            </span>
          </div>
        )}
