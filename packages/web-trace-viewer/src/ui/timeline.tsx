...
import { clsx, msToString, useMeasure } from "@/utils/ui";
import type { ConsoleEntry } from "./consoleTab.js";
import { FilmStrip } from "./filmStrip.js";
import type { FilmStripPreviewPoint } from "./filmStrip.js";
import type { Boundaries } from "./geometry.js";
import type { ActionTraceEventInContext, MultiTraceModel } from "./modelUtil.js";
import "./timeline.css";
...
export const Timeline: React.FunctionComponent<{
  ...
  highlightedAction,
  highlightedEntry,
  highlightedConsoleEntry,
  selectedTime,
  setSelectedTime,
  sdkLanguage,
}) => {
  const [measure, ref] = useMeasure<HTMLDivElement>();
  const [dragWindow, setDragWindow] = React.useState<
    | {
        startX: number;
        endX: number;
        pivot?: number;
  ...
  const onMouseDown = React.useCallback(
    (event: React.MouseEvent) => {
      ...
      if (selectedTime && Math.abs(x - leftX) < 10) {
        ...
        selectedTime &&
        time > selectedTime.minimum &&
        time < selectedTime.maximum &&
        event.clientY - ref.current.getBoundingClientRect().top < 20
      ) {
        // Move window.
  ...
  const onGlassPaneMouseMove = React.useCallback(
    (event: MouseEvent) => {
      if (!ref.current) {
        return;
      }
  ...
  return (
    <div style={{ flex: "none", borderBottom: "1px solid var(--vscode-panel-border)" }}>
      {!!dragWindow && (
        <GlassPane
