        ...
        <div className="hbox" style={{ marginLeft: 14, maxHeight: 200, overflowY: "auto" }}>
          <div className="filter-list" role="list" data-testid="status-filters">
            {[...statusFilters.entries()].map(([status, value]) => {
              return (
                <div className="filter-entry" key={status} role="listitem">
                  <label>
                    <input
                      type="checkbox"
                      checked={value}
          ...
          <div className="filter-list" role="list" data-testid="project-filters">
            {[...projectFilters.entries()].map(([projectName, value]) => {
              return (
                <div className="filter-entry" key={projectName} role="listitem">
                  <label>
                    <input
                      type="checkbox"
                      checked={value}
                      onChange={() => {
                        ...
                        if (configFile) {
                          settings.setObject(
                            `${configFile}:projects`,
                            [...copy.entries()].filter(([_, v]) => v).map(([k]) => k),
                          );
                        }
                      }}
                    />
