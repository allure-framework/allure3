...
import type { AfterActionTraceEventAttachment } from "@/types/trace";
import { CodeMirrorWrapper, lineHeight } from "@/ui/components/codeMirrorWrapper";
import { Expandable } from "@/ui/components/expandable";
import { ImageDiffView } from "@/ui/shared/imageDiffView";
import { isTextualMimeType } from "@/utils/mimeType";
import { linkifyText } from "@/utils/render";
import { clsx, useFlash } from "@/utils/ui";
...
const ExpandableAttachment: React.FunctionComponent<ExpandableAttachmentProps> = ({ attachment, reveal }) => {
  ...
  React.useEffect(() => {
    if (expanded && attachmentText === null && placeholder === null) {
      ...
      fetch(attachmentURL(attachment))
  ...
  const title = (
    <span style={{ marginLeft: 5 }} ref={ref} aria-label={attachment.name}>
      <span>{linkifyText(attachment.name)}</span>
      {hasContent && (
        <a style={{ marginLeft: 5 }} href={downloadURL(attachment)}>
          download
        </a>
      )}
    </span>
  ...
  const { diffMap, screenshots, attachments } = React.useMemo(() => {
    ...
    const diffMap = new Map<
      string,
      {
        ...
        actual: Attachment | undefined;
        diff: Attachment | undefined;
      }
    >();
  ...
  return (
    <div className="attachments-tab">
      ...
      {[...screenshots.values()].map((a, i) => {
        ...
        return (
          <div className="attachment-item" key={`screenshot-${i}`}>
            <div>
      ...
      {[...attachments.values()].map((a, i) => {
        return (
          <div className="attachment-item" key={attachmentKey(a, i)}>
            <ExpandableAttachment
              attachment={a}
              reveal={
                !!revealedAttachment && isEqualAttachment(a, revealedAttachment[0]) ? revealedAttachment : undefined
              }
            />
          </div>
        );
