        ...
        <GlassPane
          ...
          onPaneMouseMove={(event) => {
            if (!event.buttons) {
              ...
              const delta =
                orientation === "horizontal" ? event.clientX - resizing.clientX : event.clientY - resizing.clientY;
              ...
              const constrainedDelta =
                Math.min(Math.max(previous + minGap, newOffset), next - minGap) - offsets[resizing.index];
              for (let i = resizing.index; i < offsets.length; ++i) offsets[i] = offsets[i] + constrainedDelta;
              setOffsets([...offsets]);
            }
          }}
        />
