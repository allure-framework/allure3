...
export const Dialog: React.FC<React.PropsWithChildren<DialogProps>> = ({
  ...
  React.useEffect(() => {
    const onClick = (event: MouseEvent) => {
      ...
      if (!dialogRef.current.contains(event.target)) requestClose?.();
    };
...
const buildTopLeftCoord = (bounds: DOMRect, width: number): number => {
  ...
  if (leftAlignCoord.inBounds) return leftAlignCoord.value;

  const rightAlignCoord = buildTopLeftCoordWithAlignment(bounds, width, "right");

  if (rightAlignCoord.inBounds) return rightAlignCoord.value;
