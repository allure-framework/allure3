 ...
 */
import type { StackFrame } from "@/types/channels";
import type { ActionEntry, ContextEntry, PageEntry } from "@/types/entries.js";
import type { ResourceSnapshot } from "@/types/snapshot";
import type * as trace from "@/types/trace.js";
import type { Language } from "@/utils/locatorGenerators.js";
...
export class MultiTraceModel {
  ...
  constructor(contexts: ContextEntry[]) {
    ...
    this.events = ([] as (trace.EventTraceEvent | trace.ConsoleMessageTraceEvent)[]).concat(
      ...contexts.map((c) => c.events),
    );
...
export function context(action: trace.ActionTraceEvent | trace.EventTraceEvent | ResourceSnapshot): ContextEntry {
  return (action as any)[contextSymbol];
}

function nextInContext(action: trace.ActionTraceEvent): trace.ActionTraceEvent {
  return (action as any)[nextInContextSymbol];
}

export function prevInList(action: trace.ActionTraceEvent): trace.ActionTraceEvent {
  return (action as any)[prevInListSymbol];
}

export function stats(action: trace.ActionTraceEvent): { errors: number; warnings: number } {
  let errors = 0;
  let warnings = 0;
...
]);
{
  ...
  for (const method of ["page.route_from_har", "page.unroute_all", "context.route_from_har", "context.unroute_all"]) {
