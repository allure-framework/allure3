...
import type { SerializedValue } from "@/types/channels.js";
import type { ActionTraceEvent } from "@/types/trace.js";
...
import type { ActionTraceEventInContext } from "./modelUtil.js";
...
const DateTimeCallLine: React.FC<{ name: string; value: string }> = ({ name, value }) => (
  <div className="call-line">
    {name}
    <span className="call-value datetime" title={value}>
      {value}
    </span>
  </div>
);
...
function renderProperty(property: Property) {
  ...
  return (
    <div key={property.name} className="call-line">
      {property.name}:
      <span className={clsx("call-value", property.type)} title={property.text}>
        {text}
      </span>
      {["string", "number", "object", "locator"].includes(property.type) && <CopyToClipboard value={property.text} />}
    </div>
  );
}
...
function propertyToString(
  ...
  if (name === "selector") {
    return {
      text: asLocator(sdkLanguage || "javascript", event.params.selector),
      type: "locator",
      name: "locator",
    };
  }
