...
import type { ActionTraceEvent, AfterActionTraceEventAttachment } from "@/types/trace.js";
import { ToolbarButton } from "@/ui/components/toolbarButton";
import type { TreeState } from "@/ui/components/treeView";
import { TreeView } from "@/ui/components/treeView";
import { type Language, asLocator } from "@/utils/locatorGenerators.js";
import { clsx, msToString } from "@/utils/ui";
import "./actionList.css";
import type { Boundaries } from "./geometry.js";
import * as modelUtil from "./modelUtil.js";
import type { ActionTraceEventInContext, ActionTreeItem } from "./modelUtil.js";
import { testStatusIcon } from "./testUtils.js";
...
export const ActionList: React.FC<ActionListProps> = ({
  ...
  const isVisible = React.useCallback(
    (item: ActionTreeItem) => {
      return (
        !selectedTime ||
        !item.action ||
        (item.action!.startTime <= selectedTime.maximum && item.action!.endTime >= selectedTime.minimum)
      );
    },
    [selectedTime],
  );
  ...
  const onHighlightedAction = React.useCallback(
    (item: ActionTreeItem | undefined) => {
  ...
  return (
    <div className="vbox">
      {selectedTime && (
        <div className="action-list-show-all" onClick={() => setSelectedTime(undefined)}>
          <span className="codicon codicon-triangle-left"></span>Show all
        </div>
      )}
      <ActionTreeView
        name="actions"
...
export const renderAction = (
  ...
  const isSkipped =
    action.class === "Test" && action.method === "step" && action.annotations?.some((a) => a.type === "skip");
  let time = "";
  if (action.endTime) {
  ...
  return (
    <>
      <div className="action-title" title={action.apiName}>
        ...
        {parameterString &&
          (parameterString.type === "locator" ? (
            <>
              <span className="action-parameter action-locator-parameter">{parameterString.value}</span>
            ...
            <span className="action-parameter action-generic-parameter">{parameterString.value}</span>
          ))}
        {action.method === "goto" && action.params.url && (
          <div className="action-url" title={action.params.url}>
            {action.params.url}
          </div>
        )}
      ...
      {showAttachments && (
        <ToolbarButton icon="attach" title="Open Attachment" onClick={() => revealAttachment(action.attachments![0])} />
      )}
      {showDuration && !isSkipped && (
        <div className="action-duration">{time || <span className="codicon codicon-loading"></span>}</div>
      )}
      {isSkipped && (
        <span className={clsx("action-skipped", "codicon", testStatusIcon("skipped"))} title="skipped"></span>
      )}
      {showBadges && (
...
const clockDisplayString = (action: ActionTraceEvent): ActionParameterDisplayString | undefined => {
  switch (action.method) {
    case "clockPauseAt":
    case "clockSetFixedTime":
    case "clockSetSystemTime": {
      if (action.params.timeString === undefined && action.params.timeNumber === undefined) {
        return undefined;
      }
      return {
        type: "generic",
        value: new Date(action.params.timeString ?? action.params.timeNumber).toLocaleString(undefined, {
          timeZone: "UTC",
        }),
      };
    }
    ...
    case "clockRunFor": {
      if (action.params.ticksNumber === undefined && action.params.ticksString === undefined) {
        return undefined;
      }
      return {
        type: "generic",
...
const keyboardDisplayString = (action: ActionTraceEvent): ActionParameterDisplayString | undefined => {
  switch (action.method) {
    case "press":
    case "keyboardPress":
    case "keyboardDown":
...
const mouseDisplayString = (action: ActionTraceEvent): ActionParameterDisplayString | undefined => {
  switch (action.method) {
    case "click":
    case "dblclick":
    case "mouseClick":
    ...
    case "mouseWheel": {
      if (action.params.deltaX === undefined || action.params.deltaY === undefined) {
        return undefined;
      }
      return {
        type: "generic",
...
const touchscreenDisplayString = (action: ActionTraceEvent): ActionParameterDisplayString | undefined => {
  switch (action.method) {
    case "tap": {
      if (action.params.x === undefined || action.params.y === undefined) {
        return undefined;
...
const actionParameterDisplayString = (
  ...
  if (!ignoreLocator && params.selector !== undefined) {
    return {
      ...
      childDisplayString: actionParameterDisplayString(action, sdkLanguage, true),
    };
  }
