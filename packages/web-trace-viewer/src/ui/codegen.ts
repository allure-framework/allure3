 ...
 */
import type * as har from "@/types/har.js";
...
class JSCodeGen implements APIRequestCodegen {
  generatePlaywrightRequestCall(request: har.Request, body: string | undefined): string {
    ...
    if (url.searchParams.size) options.params = Object.fromEntries(url.searchParams.entries());
    ...
    if (hasOptions) params.push(this.prettyPrintObject(options));
  ...
  private prettyPrintObject(obj: any, indent = 2, level = 0): string {
    ...
    if (typeof obj !== "object") {
      if (typeof obj === "string") return this.stringLiteral(obj);
      return String(obj);
    }
    ...
    if (Array.isArray(obj)) {
      ...
      const items = obj.map((item) => `${nextSpaces}${this.prettyPrintObject(item, indent, level + 1)}`).join(",\n");
...
class PythonCodeGen implements APIRequestCodegen {
  generatePlaywrightRequestCall(request: har.Request, body: string | undefined): string {
    ...
    if (body) params.push(`data=${this.prettyPrintObject(body)}`);
    if (request.headers.length)
      params.push(
        `headers=${this.prettyPrintObject(Object.fromEntries(request.headers.map((header) => [header.name, header.value])))}`,
      );
...
class CSharpCodeGen implements APIRequestCodegen {
  ...
  private prettyPrintObject(obj: any, indent = 2, level = 0): string {
    ...
    // Handle regular objects
    if (Object.keys(obj).length === 0) return "new {}";
    const spaces = " ".repeat(level * indent);
    const nextSpaces = " ".repeat((level + 1) * indent);
...
class JavaCodeGen implements APIRequestCodegen {
  generatePlaywrightRequestCall(request: har.Request, body: string | undefined): string {
    ...
    if (body) options.push(`setData(${this.stringLiteral(body)})`);
    for (const header of request.headers)
      options.push(`setHeader(${this.stringLiteral(header.name)}, ${this.stringLiteral(header.value)})`);

    if (options.length > 0) params.push(`RequestOptions.create()\n  .${options.join("\n  .")}\n`);
...
export function getAPIRequestCodeGen(language: Language): APIRequestCodegen {
  if (language === "javascript") return new JSCodeGen();
  if (language === "python") return new PythonCodeGen();
  if (language === "csharp") return new CSharpCodeGen();
  if (language === "java") return new JavaCodeGen();
