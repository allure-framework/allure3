...
import type * as channels from "@/types/channels.js";
import { ListView } from "@/ui/components/listView";
import { ansi2html } from "@/utils/ansi2html";
...
export function useConsoleTabModel(
  ...
  const filteredEntries = React.useMemo(() => {
    ...
    return entries.filter(
      (entry) => entry.timestamp >= selectedTime.minimum && entry.timestamp <= selectedTime.maximum,
    );
    ...
    <div className="console-tab">
      <ConsoleListView
        ...
        render={(entry) => {
          ...
          const statusElement =
            entry.browserMessage || entry.browserError ? (
              <span className={clsx("codicon", "codicon-browser", errorSuffix)} title="Browser message"></span>
            ) : (
              <span className={clsx("codicon", "codicon-file", errorSuffix)} title="Runner message"></span>
            );
          ...
          return (
            <div className="console-line">
              ...
              {messageInnerHTML && (
                <span className="console-line-message" dangerouslySetInnerHTML={{ __html: messageInnerHTML }}></span>
              )}
