...
import type { ElementInfo } from "@/types/recorder";
import type { ActionTraceEvent } from "@/types/trace";
import { TabbedPaneTab } from "@/ui/components/tabbedPane";
import { Toolbar } from "@/ui/components/toolbar";
import { ToolbarButton } from "@/ui/components/toolbarButton";
import { parseAriaSnapshot } from "@/utils/ariaSnapshot";
import ConsoleAPI from "@/utils/injected/consoleApi";
import { InjectedScript } from "@/utils/injected/injectedScript";
import { Recorder } from "@/utils/injected/recorder/recorder";
import { asLocator } from "@/utils/locatorGenerators";
import type { Language } from "@/utils/locatorGenerators";
import { locatorOrSelectorAsSelector } from "@/utils/locatorParser";
import { clsx, useMeasure, useSetting } from "@/utils/ui";
import { BrowserFrame } from "./browserFrame.js";
import { type MultiTraceModel, context, prevInList } from "./modelUtil.js";
...
export const SnapshotTabsView: React.FunctionComponent<{
  ...
  return (
    <div className="snapshot-tab vbox">
      <Toolbar>
        <ToolbarButton
          className="pick-locator"
          title="Pick locator"
          icon="target"
          toggled={isInspecting}
      ...
      <SnapshotWrapper snapshotInfo={snapshotInfo}>
        <div className="snapshot-switcher">
          <iframe
            ...
            name="snapshot"
            title="DOM Snapshot"
            className={clsx(loadingRef.current.visibleIframe === 0 && "snapshot-visible")}
          ></iframe>
          <iframe
            ref={iframeRef1}
            name="snapshot"
            title="DOM Snapshot"
            className={clsx(loadingRef.current.visibleIframe === 1 && "snapshot-visible")}
          ></iframe>
        </div>
      </SnapshotWrapper>
    </div>
...
const SnapshotWrapper: React.FunctionComponent<
...
export const InspectModeController: React.FunctionComponent<{
  ...
  React.useEffect(() => {
    ...
    const parsedSnapshot =
      highlightedElement.lastEdited === "ariaSnapshot" && highlightedElement.ariaSnapshot
        ? parseAriaSnapshot(yaml, highlightedElement.ariaSnapshot)
        : undefined;
    const fullSelector =
      highlightedElement.lastEdited === "locator" && highlightedElement.locator
        ? locatorOrSelectorAsSelector(sdkLanguage, highlightedElement.locator, testIdAttributeName)
        : undefined;
    for (const { recorder, frameSelector } of recorders) {
      const actionSelector = fullSelector?.startsWith(frameSelector)
        ? fullSelector.substring(frameSelector.length).trim()
        : undefined;
...
function createRecorders(
  ...
  for (let i = 0; i < frameWindow.frames.length; ++i) {
    ...
    const frameSelector = childFrame.frameElement
      ? `${win._injectedScript.generateSelectorSimple(childFrame.frameElement, {
    ...
    createRecorders(
      ...
      testIdAttributeName,
      isUnderTest,
      parentFrameSelector + frameSelector,
      childFrame,
    );
  }
}
...
export function collectSnapshots(action: ActionTraceEvent | undefined): Snapshots {
  ...
  let beforeSnapshot: Snapshot | undefined = action.beforeSnapshot
    ? {
        action,
  ...
  const afterSnapshot: Snapshot | undefined = action.afterSnapshot
    ? {
        action,
        snapshotName: action.afterSnapshot,
      }
    : beforeSnapshot;
  const actionSnapshot: Snapshot | undefined = action.inputSnapshot
    ? {
        action,
        snapshotName: action.inputSnapshot,
        hasInputTarget: true,
      }
    : afterSnapshot;
  if (actionSnapshot) {
...
export function extendSnapshot(snapshot: Snapshot, shouldPopulateCanvasFromScreenshot: boolean): SnapshotUrls {
  ...
  const snapshotUrl = new URL(
    `snapshot/${snapshot.action.pageId}?${params.toString()}`,
    window.location.href,
  ).toString();
  const snapshotInfoUrl = new URL(
    `snapshotInfo/${snapshot.action.pageId}?${params.toString()}`,
    window.location.href,
  ).toString();
