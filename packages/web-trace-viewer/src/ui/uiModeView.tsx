...
import type * as reporterTypes from "@/types/testReporter.js";
import "@/ui/common.css";
import { SplitView } from "@/ui/components/splitView";
import { Toolbar } from "@/ui/components/toolbar";
import { ToolbarButton } from "@/ui/components/toolbarButton";
import type { XtermDataSource } from "@/ui/components/xtermWrapper";
import { XtermWrapper } from "@/ui/components/xtermWrapper";
import { TeleSuite } from "@/utils/isomorphic/teleReceiver.js";
import type { TeleTestCase } from "@/utils/isomorphic/teleReceiver.js";
import {
  TeleSuiteUpdater,
  type TeleSuiteUpdaterProgress,
  type TeleSuiteUpdaterTestModel,
} from "@/utils/isomorphic/teleSuiteUpdater.js";
import { TestServerConnection, WebSocketTestServerTransport } from "@/utils/isomorphic/testServerConnection.js";
import { TestTree, statusEx } from "@/utils/isomorphic/testTree.js";
import type { TreeItem } from "@/utils/isomorphic/testTree.js";
import { clsx, settings, useSetting } from "@/utils/ui";
import { DefaultSettingsView } from "./defaultSettingsView.js";
import { GitCommitInfoProvider } from "./errorsTab.js";
import { LLMProvider } from "./llm.js";
import type { SourceLocation } from "./modelUtil.js";
import { SettingsView } from "./settingsView.js";
import { FiltersView } from "./uiModeFiltersView.js";
import { TestListView } from "./uiModeTestListView.js";
import { TraceView } from "./uiModeTraceView.js";
...
export const UIModeView: React.FC<{}> = ({}) => {
  ...
  const [statusFilters, setStatusFilters] = React.useState<Map<string, boolean>>(
    new Map([
      ["passed", false],
      ["failed", false],
      ["skipped", false],
  ...
  const [watchedTreeIds, setWatchedTreeIds] = React.useState<{
    value: Set<string>;
  }>({ value: new Set() });
  ...
  React.useEffect(() => {
    ...
    const selectedProjects = config.configFile
      ? settings.getObject<string[] | undefined>(`${config.configFile}:projects`, undefined)
      : undefined;
  ...
  const runTests = React.useCallback(
    (mode: "queue-if-busy" | "bounce-if-busy", testIds: Set<string>) => {
  ...
  const installBrowsers = React.useCallback(
    (e: React.MouseEvent) => {
      ...
      <div className="vbox ui-mode">
        {!hasBrowsers && (
          <dialog ref={dialogRef}>
            <div className="title">
              <span className="codicon codicon-lightbulb"></span>Install browsers
            </div>
            <div className="body">
              ...
              <br></br>
              <button className="button" onClick={installBrowsers}>
                Install
              </button>
              <button className="button secondary" onClick={closeInstallDialog}>
                Dismiss
              </button>
            </div>
          </dialog>
          ...
          main={
            <div className="vbox">
              <div className={clsx("vbox", !isShowingOutput && "hidden")}>
                <Toolbar>
                  <div className="section-title" style={{ flex: "none" }}>
                    Output
                  </div>
                  <ToolbarButton
                    icon="circle-slash"
                ...
                <GitCommitInfoProvider gitCommitInfo={testModel?.config.metadata["git.commit.info"]}>
                  <TraceView
                    ...
                    onOpenExternally={(location) =>
                      testServerConnection?.openNoReply({
                        location: {
                          file: location.file,
                          line: location.line,
            ...
            <div className="vbox ui-mode-sidebar">
              <Toolbar noShadow={true} noMinHeight={true}>
                ...
                <div style={{ position: "relative" }}>
                  <ToolbarButton
                    icon={"terminal"}
                    title={`Toggle output — ${isMac ? "⌃`" : "Ctrl + `"}`}
                    toggled={isShowingOutput}
                    onClick={() => {
                      setIsShowingOutput(!isShowingOutput);
                    }}
                  />
              ...
              <Toolbar noMinHeight={true}>
                ...
                {!isRunningTest && progress && (
                  <div data-testid="status-line" className="status-line">
                    <div>
                ...
                {isRunningTest && progress && (
                  <div data-testid="status-line" className="status-line">
                    <div>
                      Running {progress.passed}/{runningState.testIds.size} passed (
                ...
                <ToolbarButton
                  icon="play"
                  title="Run all — F5"
                  onClick={() => runTests("bounce-if-busy", visibleTestIds)}
                  disabled={isRunningTest || isLoading}
                ></ToolbarButton>
                <ToolbarButton
                  icon="debug-stop"
                  title={`Stop — ${isMac ? "⇧F5" : "Shift + F5"}`}
                  onClick={() => testServerConnection?.stopTests({})}
                  disabled={!isRunningTest || isLoading}
                ></ToolbarButton>
                <ToolbarButton
                  icon="eye"
              ...
              {showTestingOptions && (
                <>
                  <Toolbar
                    ...
                    onClick={() => setTestingOptionsVisible(!testingOptionsVisible)}
                  >
                    <span
                  ...
                  {testingOptionsVisible && (
                    <SettingsView
                      settings={[
                        ...
                        {
                          value: updateSnapshots,
                          set: setUpdateSnapshots,
