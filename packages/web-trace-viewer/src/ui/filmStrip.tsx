...
import type { PageEntry } from "@/types/entries.js";
import type { Language } from "@/utils/locatorGenerators.js";
import { upperBound, useMeasure } from "@/utils/ui";
import { renderAction } from "./actionList.js";
import "./filmStrip.css";
import type { Boundaries, Size } from "./geometry.js";
...
export const FilmStrip: React.FunctionComponent<{
  ...
  if (previewPoint !== undefined && screencastFrames && screencastFrames.length) {
    const previewTime =
      boundaries.minimum + ((boundaries.maximum - boundaries.minimum) * previewPoint.x) / measure.width;
  ...
  return (
    <div className="film-strip" ref={ref}>
      ...
      {previewPoint?.x !== undefined && (
        ...
        >
          {previewPoint.action && (
            <div className="film-strip-hover-title">{renderAction(previewPoint.action, previewPoint)}</div>
          )}
