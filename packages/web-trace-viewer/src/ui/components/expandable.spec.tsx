...
test("should render collapsed", async ({ mount }) => {
  const component = await mount(
    <Expandable expanded={false} setExpanded={() => {}} title="Title">
      Details text
    </Expandable>,
  );
...
test("should render expanded", async ({ mount }) => {
  const component = await mount(
    <Expandable expanded={true} setExpanded={() => {}} title="Title">
      Details text
    </Expandable>,
  );
  await expect(component.locator("text=Title")).toBeVisible();
...
test("click should expand", async ({ mount }) => {
  let expanded = false;
  const component = await mount(
    <Expandable expanded={false} setExpanded={(e) => (expanded = e)} title="Title">
      Details text
    </Expandable>,
  );
