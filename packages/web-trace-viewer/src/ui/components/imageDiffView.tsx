...
import { useMeasure } from "@/utils/ui.js";
...
export const ImageDiffView: React.FC<{
  ...
  const [mode, setMode] = React.useState<"diff" | "actual" | "expected" | "slider" | "sxs">(
    diff.diff ? "diff" : "actual",
  );
      ...
      {isLoaded && (
        <>
          <div data-testid="test-result-image-mismatch-tabs" style={{ display: "flex", margin: "10px 0 20px" }}>
            {diff.diff && (
              <div
                style={{ ...modeStyle, fontWeight: mode === "diff" ? 600 : "initial" }}
            ...
            <div
              style={{ ...modeStyle, fontWeight: mode === "actual" ? 600 : "initial" }}
              onClick={() => setMode("actual")}
            >
              Actual
            </div>
            <div
              style={{ ...modeStyle, fontWeight: mode === "expected" ? 600 : "initial" }}
              onClick={() => setMode("expected")}
            >
              {expectedImageTitle}
            </div>
            <div style={{ ...modeStyle, fontWeight: mode === "sxs" ? 600 : "initial" }} onClick={() => setMode("sxs")}>
              Side by side
            </div>
            <div
              style={{ ...modeStyle, fontWeight: mode === "slider" ? 600 : "initial" }}
          ...
          >
            ...
            {diff.diff && mode === "slider" && (
              <ImageDiffSlider
                ...
                canvasHeight={fitHeight}
                scale={scale}
                expectedTitle={expectedImageTitle}
              />
            )}
            {diff.diff && mode === "sxs" && (
              <div style={{ display: "flex" }}>
                <ImageWithSize
                  image={expectedImage}
                  title={expectedImageTitle}
                  hideSize={hideDetails}
                  canvasWidth={sxsScale * imageWidth}
                  canvasHeight={sxsScale * imageHeight}
                  scale={sxsScale}
                />
                <ImageWithSize
                  image={showSxsDiff ? diffImage : actualImage}
                  title={showSxsDiff ? "Diff" : "Actual"}
                  onClick={() => setShowSxsDiff(!showSxsDiff)}
                  hideSize={hideDetails}
                  canvasWidth={sxsScale * imageWidth}
                  canvasHeight={sxsScale * imageHeight}
                  scale={sxsScale}
          ...
          {!hideDetails && (
            <div style={{ alignSelf: "start", lineHeight: "18px", marginLeft: "15px" }}>
              <div>
                {diff.diff && (
                  <a target="_blank" href={diff.diff.attachment.path} rel="noreferrer">
                    {diff.diff.attachment.name}
                  </a>
                )}
              </div>
              <div>
                <a target={noTargetBlank ? "" : "_blank"} href={diff.actual!.attachment.path} rel="noreferrer">
                  {diff.actual!.attachment.name}
                </a>
              </div>
              <div>
                <a target={noTargetBlank ? "" : "_blank"} href={diff.expected!.attachment.path} rel="noreferrer">
                  {diff.expected!.attachment.name}
                </a>
              </div>
            </div>
          )}
...
const ImageWithSize: React.FunctionComponent<{
  ...
  return (
    <div style={{ flex: "none", display: "flex", alignItems: "center", flexDirection: "column" }}>
      {!hideSize && (
        <div style={{ margin: 5 }}>
