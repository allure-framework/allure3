...
import { ansi2html } from "@/utils/ansi2html.js";
import { kWebLinkRe, useMeasure } from "@/utils/ui.js";
import type { CodeMirror } from "./codeMirrorModule.js";
...
export const CodeMirrorWrapper: React.FC<SourceProps> = ({
  ...
  const [modulePromise] = React.useState<Promise<CodeMirror>>(import("./codeMirrorModule.js").then((m) => m.default));
  ...
  React.useLayoutEffect(() => {
    ...
    if (valueChanged || JSON.stringify(highlight) !== JSON.stringify(codemirrorRef.current!.highlight)) {
      ...
      for (const h of highlight || []) {
        ...
        if (h.type === "error") {
          ...
          widgets.push(
            codemirror.addLineWidget(h.line, errorWidgetElement, {
              above: true,
              coverGutter: false,
            }),
          );
    ...
    if (onChange) {
      changeListener = () => onChange(codemirror.getValue() as string);
  ...
  return (
    <div data-testid={dataTestId} className="cm-wrapper" ref={codemirrorElement} onClick={onCodeMirrorClick}></div>
  );
};
...
function onCodeMirrorClick(event: React.MouseEvent) {
  ...
  if (event.target.classList.contains("cm-linkified")) {
    ...
    event.target.nextElementSibling?.classList.contains("cm-url")
  ) {
    // 'markdown' mode
...
function defineCustomMode(cm: CodeMirror) {
  ...
  (cm as any).defineSimpleMode("text/linkified", {
    start: [{ regex: kWebLinkRe, token: "linkified" }],
  });
}
