...
import { clsx, scrollIntoViewIfNeeded } from "@/utils/ui.js";
import "./treeView.css";
...
export function TreeView<T extends TreeItem>({
  ...
  return (
    <div
      className={clsx("tree-view vbox", `${name}-tree-view`)}
      role={"tree"}
      data-testid={dataTestId || `${name}-tree`}
    >
      <div
...
export function TreeItemHeader<T extends TreeItem>({
  ...
  return (
    ...
    >
      ...
      >
        {indentation
          ? new Array(indentation).fill(0).map((_, i) => <div key={`indent-${i}`} className="tree-view-indent"></div>)
          : undefined}
        <div
          aria-hidden="true"
        ...
        {icon && (
          <div
            className={`codicon ${iconed}`}
            style={{ minWidth: 16, marginRight: 4 }}
            aria-label={`[${iconed.replace("codicon", "icon")}]`}
          ></div>
        )}
        {typeof rendered === "string" ? (
          <div style={{ textOverflow: "ellipsis", overflow: "hidden" }}>{rendered}</div>
        ) : (
          rendered
        )}
      </div>
