...
export const TabbedPane: React.FunctionComponent<{
  ...
  return (
    <div className="tabbed-pane" data-testid={dataTestId}>
      <div className="vbox">
        <Toolbar>
          {leftToolbar && (
            <div
          ...
          {mode === "default" && (
            <div style={{ flex: "auto", display: "flex", height: "100%", overflow: "hidden" }} role="tablist">
              {[
                ...tabs.map((tab) => (
                  <TabbedPaneTab
                    key={tab.id}
        ...
        {tabs.map((tab) => {
          ...
          if (selectedTab === tab.id) {
            return (
              <div key={tab.id} id={`${id}-${tab.id}`} role="tabpanel" aria-label={tab.title} className={className}>
                {tab.render!()}
              </div>
            );
          }
