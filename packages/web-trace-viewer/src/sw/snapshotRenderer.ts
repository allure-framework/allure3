...
import { escapeHTML, escapeHTMLAttribute } from "@/utils/string.js";
...
export class SnapshotRenderer {
  ...
  render(): RenderedFrameSnapshot {
    ...
    const visit = (
      ...
      parentAttrs: [string, string][] | undefined,
    ) => {
      // Text node.
      if (typeof n === "string") {
      ...
      if (isSubtreeReferenceSnapshot(n)) {
        ...
        const isSourceInsidePictureWithCurrentSrc =
          nodeName === "SOURCE" && parentTag === "PICTURE" && parentAttrs?.some((a) => a[0] === kCurrentSrcAttribute);
...
function snapshotScript(viewport: ViewportSize, ...targetIds: (string | undefined)[]) {
  function applyPlaywrightAttributes(
    unwrapPopoutUrl: (url: string) => string,
    viewport: ViewportSize,
    ...
    const kPointerWarningTitle =
      "Recorded click position in absolute coordinates did not" +
