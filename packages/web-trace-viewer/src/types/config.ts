...
//
//     this.globalSetups = (Array.isArray(userConfig.globalSetup) ? userConfig.globalSetup : [userConfig.globalSetup])
//       .map((s) => resolveScript(s, configDir))
//       .filter((script) => script !== undefined);
//     this.globalTeardowns = (
//       Array.isArray(userConfig.globalTeardown) ? userConfig.globalTeardown : [userConfig.globalTeardown]
//     )
//       .map((s) => resolveScript(s, configDir))
//       .filter((script) => script !== undefined);
//
...
//         configCLIOverrides.debug ? 1 : undefined,
...
//       reporter: takeFirst(configCLIOverrides.reporter, resolveReporters(userConfig.reporter, configDir), [
//         [defaultReporter],
//       ]),
...
//       if (key.startsWith("@")) (this.config as any)[key] = (userConfig as any)[key];
//     }
//
//     (this.config as any)[configInternalSymbol] = this;
//
//     const workers = takeFirst(
//       configCLIOverrides.debug ? 1 : undefined,
...
//       (p) => new FullProjectInternal(configDir, userConfig, this, p, this.configCLIOverrides, packageJsonDir),
//     );
...
//         path.join(packageJsonDir, "test-results"),
//       ),
...
//       timeout: takeFirst(
//         configCLIOverrides.debug ? 0 : undefined,
...
//     );
//     this.ignoreSnapshots = takeFirst(
