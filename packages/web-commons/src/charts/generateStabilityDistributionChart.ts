 ...
 * records first two transition indices, and sums weighted contribution to instability (Rule 7).
 * F→P (recovery) does not contribute.
 */
const computeTransitionsInRange = (
  ...
  lastEdge: number,
  denominator: number,
): { transitionCount: number; firstTransitionIdx: number; secondTransitionIdx: number; instabilitySum: number } => {
  ...
  for (let i = firstEdge; i <= lastEdge; i++) {
    ...
    if (!(statuses[i - 1] === "failed" && statuses[i] === "passed")) {
      instabilitySum += i / denominator;
    }
  }

  return { transitionCount, firstTransitionIdx, secondTransitionIdx, instabilitySum };
};
...
export const getStabilityScore = (
  ...
  // Rule 3: that block is at the very end and all "passed" → test is stable
  if (stabilBlock !== null && stabilBlock.endIndex === effectiveSequenceLength - 1 && stabilBlock.status === "passed") {
    return 1;
  }
  ...
  const { transitionCount, firstTransitionIdx, secondTransitionIdx, instabilitySum } = computeTransitionsInRange(
    statuses,
    firstEdgeAfterStabilBlock,
    lastEdge,
    denominator,
  );
  ...
  // Rule 7: weighted sum of transition contributions (more recent = higher weight)
  return instabilitySum;
};
