...
import { getSuccessRate, includedInSuccessRate } from "@allurereport/core-api";
...
const addLeafToGroupFn = (group: Group, leaf: Leaf): void => {
  ...
  group.eligibleCount += includedInSuccessRate.has(leaf.status) ? 1 : 0;
...
const calculateSubtreeMetrics = (node: ExtendedTreeMapNode): SubtreeMetrics => {
  if (!node.children || node.children.length === 0) {
    ...
    return {
      totalTests: node.status ? 1 : 0,
      eligibleCount: node.status && includedInSuccessRate.has(node.status) ? 1 : 0,
  ...
  for (const child of node.children) {
    ...
    eligibleCount += childMetrics.eligibleCount;
    passedTests += childMetrics.passedTests;
    failedTests += childMetrics.failedTests;
    otherTests += childMetrics.otherTests;
  }

  return { totalTests, eligibleCount, passedTests, failedTests, otherTests };
};
...
export const createSuccessRateDistributionTreeMap = (testResults: TestResult[]): ExtendedTreeMapNode => {
    ...
    (node, isGroup) => {
      ...
      if (isGroup) {
        ...
        return {
          ...baseNode,
          status: leaf.status,
          eligibleCount: includedInSuccessRate.has(leaf.status) ? 1 : 0,
  ...
  return transformTreeMapNode<ExtendedTreeMapNode>(convertedTree, (node) => {
    ...
    const { totalTests: _totalTests, ...restSubtreeMetrics } = subtreeMetrics;
