...
const sectionMap: Record<string, { name: string; logo: string }> = {
  "": { name: "Report", logo: allureIcons.reportLogo },
...
export const SectionPicker = () => {
  const selectedSection = sectionMap[currentSection.value] || sectionMap[""];
  return (
    <Menu
      size="m"
    ...
    >
      <Menu.Section>
        {availableSections.value?.map((value) => (
