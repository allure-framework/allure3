...
import { flatTree } from "../../src/stores/keyboard";
import {
  allTreesCollapsed,
  collapsedTrees,
  expandedTrees,
...
describe("stores > tree > collapse all", () => {
  ...
  it.each([
    ...
    const expandedGroups = flatTree.value.filter((row) => row.kind === "group" && row.isExpanded).map((row) => row.id);

    expect(expandedGroups).toEqual([]);

    setAllTreesOpened(true);

    const collapsedGroups = flatTree.value
