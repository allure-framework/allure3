#!/usr/bin/env node
/**
 * Generates report.html for API mode: same pre-built web-awesome assets as static report,
 * with inline script that sets apiBaseUrl and launchId from URL params.
 * Output: report-app/dist/report.html (run after build).
 */
...
const reportAppDir = join(baseDir, "..");
const webAwesomeDist = join(reportAppDir, "..", "web-awesome", "dist", "multi");
const manifestPath = join(webAwesomeDist, "manifest.json");
const outDir = join(reportAppDir, "dist");
const outPath = join(outDir, "report.html");

const ASSETS_PREFIX = "/report-assets";

function createStylesLinkTag(src) {
  return `<link rel="stylesheet" type="text/css" href="${ASSETS_PREFIX}/${src}">`;
}

function createScriptTag(src) {
  return `<script src="${ASSETS_PREFIX}/${src}"></script>`;
}

function createFontLinkTag(src) {
  return `<link rel="preload" href="${ASSETS_PREFIX}/${src}" as="font" type="font/woff" crossorigin /> `;
}
...
async function main() {
  ...
  try {
    ...
    if (err.code === "ENOENT") {
      console.error(
        "Error: web-awesome/dist/multi/manifest.json not found.\n" +
          "  Run: yarn workspace @allurereport/web-awesome run build:prod:multi\n" +
          "  If dist is owned by root, run once: sudo rm -rf packages/web-awesome/dist"
      );
    }
    throw err;
  }
...
<head>
  ...
  <base href="${ASSETS_PREFIX}/">
  ${headTags.join("\n  ")}
  <script>
    window.allureReportOptions = ${JSON.stringify(baseReportOptions)};
  </script>
</head>
<body>
  <div id="app"></div>
  ...
  <script>
    (function(){
      ...
      var launchId = p.get('launch_id') || p.get('launchId') || '';
      var apiBaseUrl = p.get('apiBaseUrl') || window.location.origin;
      window.allureReportOptions = window.allureReportOptions || {};
      window.allureReportOptions.apiBaseUrl = apiBaseUrl.replace(/\\/$/, '');
      window.allureReportOptions.launchId = launchId;
