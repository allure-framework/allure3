  ...
  test.describe("test results", () => {
    ...
    test.describe("titles", () => {
      test("retry titles of tests with timestampts have prefixes and timestamps", async () => {
        ...
        await expect(retryAt0.textLocator).toHaveText(/^Attempt 1 of 3 – \d+\/\d+\/\d+ at \d+:\d+:\d+$/);
        await expect(retryAt1.textLocator).toHaveText(/^Attempt 2 of 3 – \d+\/\d+\/\d+ at \d+:\d+:\d+$/);
      });
      ...
      test("retry titles of tests with no timestampts have prefixes", async () => {
        ...
        await expect(retryAt0.textLocator).toHaveText("Attempt 1 of 3");
        await expect(retryAt1.textLocator).toHaveText("Attempt 2 of 3");
      });
    });
  });
});
