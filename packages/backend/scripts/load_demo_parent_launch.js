#!/usr/bin/env node
/**
 * Load demo data with parentLaunchId: creates a parent launch with variables/metadata,
 * then creates child launches from allure2_custom/demo jobs that have failed tests.
 * Usage:
 *   API_URL=http://localhost:3000/api/v1 node scripts/load_demo_parent_launch.js
 *   DEMO_ROOT=/path/to/allure2_custom/demo node scripts/load_demo_parent_launch.js
 */
...
// allure2_custom is sibling of allure3_custom: ../../.. = allure3_custom root, ../allure2_custom/demo
...
// Jobs with failed tests (from failed-tests.json total > 0)
...
/** Extract human-readable env name from report metadata (before creating launch). */
...
async function main() {
  if (!existsSync(DEMO_ROOT)) {
    console.error(`Demo root not found: ${DEMO_ROOT}`);
    process.exit(1);
  }

  console.log('Loading demo with parentLaunchId');
  console.log('Demo root:', DEMO_ROOT);

  // 1. Create parent launch with custom variables and metadata
  ...
  // 2. Add metadata (allure_environment) to parent
  ...
  // 3. Create child launches and upload results
  const childIds = [];
  for (const jobName of JOBS_WITH_FAILURES) {
    ...
    console.log(`\n3. Child: ${jobName}${displayName ? ` (display: ${displayName})` : ''}`);
    const childId = await createLaunch({
      parent_launch_id: parentId,
      environment_name: envName
    });
    childIds.push({ job: jobName, id: childId });
  ...
  console.log(`Child launches: ${childIds.map((c) => `${c.job}=${c.id}`).join(', ')}`);
