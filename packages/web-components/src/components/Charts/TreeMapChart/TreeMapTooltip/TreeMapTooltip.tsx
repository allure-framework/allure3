    ...
    return (
      <div className={styles["tree-map-tooltip"]}>
        <div className={styles["tree-map-tooltip-title"]}>
          ...
          <Text size="s" type="ui">{title}:</Text>
          <Text size="s" type="ui" bold>{formattedValue}</Text>
        </div>
        {rows && <div className={styles["tree-map-tooltip-rows"]}>
          {rows.map((row) => (
            <Text size="s" type="ui" className={styles["tree-map-tooltip-row"]} key={row}>{row}</Text>
          ))}
        </div>}
      </div>
    );
