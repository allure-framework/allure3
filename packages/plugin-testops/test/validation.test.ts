...
import { validateExecutableName } from "../src/utils/validation.js";

describe("validateExecutableName", () => {
  it.each([
    "",
    "   ",
    "\t\n",
    ...
    "\u0085",
    "\u0000",
    "\ufffd", // Intentional boundary: replacement-decoded binary content is not executable name.
    "\ud800",
    "\udc00",
  ])("rejects %j", (name) => {
    expect(validateExecutableName(name)).toBe(false);
  });

  it("preserves valid Unicode", () => {
    expect(validateExecutableName("Пройдено ✅ 𐀀")).toBe(true);
  });

  it("preserves surrounding whitespace", () => {
    expect(validateExecutableName("  valid name  ")).toBe(true);
  });
});
