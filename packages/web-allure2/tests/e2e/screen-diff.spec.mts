...
test.describe("Screen Diff", () => {
  test("renders the dedicated screen-diff block and persists the selected mode", async ({ page }) => {
    ...
    await expect(page.locator(".pane__section-title", { hasText: screenDiff.blockTitle })).toBeVisible();
    ...
    await expect(page.locator('.attachment-row[data-type="application/vnd.allure.image.diff"]')).toHaveCount(3);
    ...
    await expect(checkoutPreview.locator('.screen-diff__switchers input[value="overlay"]')).toBeChecked();
    ...
    await expect(checkoutPreview.locator('.screen-diff__switchers input[value="diff"]')).toBeChecked();
  });
});
