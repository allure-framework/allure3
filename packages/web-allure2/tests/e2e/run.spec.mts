...
test.describe("Run errors and attachments", () => {
  test("marks an otherwise green report unsuccessful without changing test statistics", async ({ page }) => {
    await test.step("Open the overview with a passed test and a global error", async () => {
      ...
      await expect(page.locator(".run-status-banner__title")).toHaveText("Run completed with errors");
      await expect(page.locator(".run-status-banner__description")).toContainText("All 1 reported tests passed");
    ...
    await test.step("Review the run error and its attachment", async () => {
      ...
      await expect(page.locator(".run-view__error-message")).toHaveText(fixtures.globalsGreen.error);
      ...
      await expect(previewContainerFor(attachment).locator(".attachment-preview__text")).toContainText(
        fixtures.globalsGreen.attachmentContent,
      );
    });
  });
  ...
  test("explains when a run failed before any tests were reported", async ({ page }) => {
    await test.step("Open a fixture-only failure with zero test results", async () => {
      ...
      await expect(page.locator(".run-status-banner__title")).toHaveText("Run failed before tests were reported");
    ...
    await test.step("Show the unmodeled fixture error without a phantom test", async () => {
      ...
      await expect(page.locator(".run-view__error-message")).toHaveText(fixtures.globalsNoTests.error);
  ...
  test("keeps attachment-only runs neutral and makes evidence available", async ({ page }) => {
    ...
    await test.step("Open the global attachment from the neutral Run page", async () => {
      ...
      await attachment.click();
      await expect(previewContainerFor(attachment).locator(".attachment-preview__text")).toContainText(
        fixtures.globalsAttachments.attachmentContent,
      );
    });
  });
  ...
  test("shows attachment timestamps and orders evidence by time then name", async ({ page }) => {
    ...
    await expect(
      page.locator(".attachment-row", { hasText: "Alpha legacy log" }).locator(".attachment-row__details"),
    ).toHaveCount(0);
  });
