...
for (const mode of reportModes) {
  test.describe(`Test Result Detail (${mode})`, () => {
    ...
    test("links only whole absolute parameter values", async ({ page }) => {
      ...
      await expect(absoluteUrl.getByRole("link", { name: "https://example.org/docs" })).toHaveAttribute(
        "href",
      ...
      await expect(
        page.locator(".environment", { hasText: "Relative URL: ./index.html#/graphs" }).getByRole("link"),
      ).toHaveCount(0);
      await expect(
        page.locator(".environment", { hasText: "Protocol-relative URL: //example.org/docs" }).getByRole("link"),
      ).toHaveCount(0);
      await expect(page.locator(".environment", { hasText: "Plain text: API - Local" }).getByRole("link")).toHaveCount(
        0,
      );
      await expect(
        page.locator(".environment", { hasText: "Embedded URL: Open https://example.org/docs" }).getByRole("link"),
      ).toHaveCount(0);
      await expect(
        page.locator(".environment", { hasText: "Dangerous URL: javascript:alert(1)" }).getByRole("link"),
      ).toHaveCount(0);
    });
    ...
    test("detects only absolute links in step names", async ({ page }) => {
      ...
      await expect(entityEncodedUrl.getByRole("link", { name: "https://a.org/x?a=1&amp;b=2" })).toHaveAttribute(
        "href",
        "https://a.org/x?a=1&amp;b=2",
      );
    ...
    test("renders history and retries for retried results", async ({ page }) => {
      ...
      await expect(page.locator(".test-result-history__success-rate")).toContainText("Success rate");
      ...
      await expect(page.locator(".preformated-text code").first()).toContainText("Element not found");
    });
    ...
    test("shows clipboard tooltip feedback for fullname copy", async ({ page }) => {
      ...
      await copyButton.click();
      await expect(page.locator(".tooltip.tooltip_position_left")).toContainText("Can not copy value to clipboard");
    });
