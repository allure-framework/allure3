...
const scanTargets: AccessibilityTarget[] = [
  ...
  },
  {
    ...
    open: async (page) => {
      ...
      await expect.poll(() => page.locator(".timeline__plot a").count()).toBeGreaterThan(10);
    },
  },
  {
    ...
    open: async (page) => {
      ...
      await expect(page.locator(".test-result__name")).toContainText(uiDemo.cases.failedPullRequest);
    },
  },
  {
    ...
    open: async (page) => {
      ...
      const attachmentRow = attachmentStep.locator(".attachment-row", { hasText: uiDemo.htmlAttachmentName }).first();
...
const isApcaCoveredStatusPair = (node: AxeNode): boolean =>
  node.target.includes(".test-result__status > .label_status_failed.label");

const filterIgnoredViolations = (violations: AxeViolation[]): AxeViolation[] =>
  violations
    .map((violation) => {
      ...
      // This specific failed-status badge pair is intentionally covered by
      // APCA token checks; keep axe color-contrast active for every other node.
      return {
        ...violation,
        nodes: violation.nodes.filter((node) => !isApcaCoveredStatusPair(node)),
      };
    })
...
const formatViolation = (violation: AxeViolation): string => {
  ...
  return `${violation.id} (${violation.impact ?? "unknown"}): ${violation.nodes.length} nodes, ${targets}`;
};
...
const attachViolationSteps = async (target: AccessibilityTargetName, violations: AxeViolation[]) => {
  for (const violation of violations) {
    await step(
      `axe ${violation.id}: ${violation.nodes.length} affected node${violation.nodes.length === 1 ? "" : "s"}`,
      async (context) => {
        ...
        await context.parameter("targets", violation.nodes.map((node) => node.target.join(" ")).join("\n"));
...
test.describe("Accessibility Smoke", () => {
  for (const target of scanTargets) {
    test(`${target.name} has no automated accessibility violations`, async ({ page }) => {
      ...
      await attachViolationSteps(target.name, violations);

      assertNoViolations(target.name, violations);
    });
  }
});
