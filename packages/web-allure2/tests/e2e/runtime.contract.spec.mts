...
const collectResultLabelGaps = async (directory: string): Promise<Array<{ file: string; missing: string[] }>> => {
  ...
  const gaps = await Promise.all(
    resultFiles.map(async (file) => {
      ...
      const missing = ["epic", "feature", "story", "host", "thread", "package"].filter((label) => !labels.has(label));
...
test.describe("Runtime Contract", () => {
  test("removes legacy globals and keeps the report scripts free of legacy package names", async ({ page }) => {
    ...
    const bundle = (await Promise.all(bundleFiles.map((file) => fs.readFile(file, "utf8")))).join("\n");
  ...
  test("keeps the demo and raw fixtures fully labeled and timeline-friendly", async () => {
    ...
    expect(
      fixtureChecks.filter(
        ({ gaps, tooShort, span }) => gaps.length > 0 || tooShort.length > 0 || span > 10 * 60 * 1000,
      ),
    ).toEqual([]);
  });
  ...
  test("keeps Playwright trace fixtures as real zip archives", async () => {
    const traceFixtures = [
      path.join(generatorRoot, "tests", "fixtures", "raw", "playwright-trace", "playwright-trace-archive.zip"),
      path.join(generatorRoot, "tests", "fixtures", "raw", "ui-demo", "playwright-trace-archive.zip"),
  ...
  test("does not bundle a Playwright trace viewer into reports", async () => {
    await expect(
      pathExists(
        path.join(reportRoot, fixtures.playwrightTrace.name, REPORT_MODES.DIRECTORY, "playwright-trace-viewer"),
      ),
    ).resolves.toBe(false);
    await expect(
      pathExists(
