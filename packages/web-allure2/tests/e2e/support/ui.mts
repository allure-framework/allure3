...
export const readWidgetColumns = (page: Page): Promise<string[][]> =>
  page.locator(".widgets-grid__col").evaluateAll((columns) =>
    columns.map((column) =>
      Array.from(column.querySelectorAll<HTMLElement>(".widget__title")).map((title) => {
        ...
        return (primaryText?.textContent ?? title.textContent ?? "").replace(/\s+/g, " ").trim();
      }),
    ),
  );

export const sortTreeBy = async (page: Page, sorterName: string, direction: "asc" | "desc" = "desc"): Promise<void> => {
