...
for (const mode of reportModes) {
  test.describe(`Tree Shared (${mode})`, () => {
    test("suites supports search, marks, filters, sorting, and keyboard navigation", async ({ page }) => {
      ...
      await expect(page.locator(".node__leaf", { hasText: uiDemo.cases.failedPullRequest })).toBeVisible();
      ...
      await expect(page.locator(".node__leaf", { hasText: uiDemo.cases.flakyNewFailedIssue })).toBeVisible();
      await toggleMarkFilter(page, "newFailed");

      await searchInput.fill(uiDemo.cases.retriesStatusChangeIssue);
      await toggleMarkFilter(page, "retriesStatusChange");
      await expect(page.locator(".node__leaf", { hasText: uiDemo.cases.retriesStatusChangeIssue })).toBeVisible();
      await toggleMarkFilter(page, "retriesStatusChange");
      ...
      await expect(pullRequestsGroup.locator(":scope > .node__children > .node__leaf .node__name").first()).toHaveText(
      ...
      await expect(page.locator(".test-result__name")).toContainText(uiDemo.cases.passedPullRequest);

      await page.locator(".pane__title-text").click();
      await page.keyboard.press("ArrowDown");
      await expect.poll(() => currentRoute(page)).not.toBe(firstSelectedRoute);
      await expect(page.locator(".test-result__name")).not.toContainText(uiDemo.cases.passedPullRequest);
    ...
    test("behaviors, categories, and packages expose their tree-specific affordances", async ({ page }) => {
      ...
      await expect(page.locator(".tree__download")).toHaveAttribute("data-download", "data/behaviors.csv");
      ...
      await expect(page.locator(".node__name", { hasText: uiDemo.categories.statusMessage })).toBeVisible();
      await expect(page.locator(".tree__download")).toHaveAttribute("data-download", "data/categories.csv");
    ...
    test("collapsed groups stay collapsed when selecting leaves in other groups", async ({ page }) => {
      await openReport(page, { fixture: uiDemo.name, mode, route: "suites" });
      ...
      await firstGroup.locator(":scope > .node__children > .node__leaf .node__title").first().click();
      ...
      await secondGroup.locator(":scope > .node__children > .node__leaf .node__title").first().click();
    ...
    test("keyboard navigation expands and collapses the selected group", async ({ page }) => {
      ...
      await expect
        ...
        .toBe(firstChildParentUid ? `suites/${firstChildParentUid}/${firstChildUid}/` : `suites/${firstChildUid}`);
...
test.describe("Tree Persistence", () => {
  test("directory mode preserves tree sorting and filter settings after reload", async ({ page }) => {
    ...
    await expect(
      page.locator('.sorter__item[data-name="sorter.duration"] .sorter__icon.lineArrowsSortLineDesc.sorter_enabled'),
    ).toBeVisible();
    ...
    await expect
      .poll(() =>
        page.evaluate(() => {
          const settings = JSON.parse(window.localStorage.getItem("ALLURE_REPORT_SETTINGS_SUITES") ?? "{}");
          return {
  ...
  test("directory mode persists split-pane sizes after resize", async ({ page }) => {
    ...
    await page.mouse.move(gutterBox.x + gutterBox.width / 2 + 120, gutterBox.y + gutterBox.height / 2);
    await page.mouse.up();
    ...
    const restoredSplit = await page.evaluate(() => {
      ...
      return {
        leftPercent: leftWidth + rightWidth > 0 ? Math.round((leftWidth / (leftWidth + rightWidth)) * 100) : 0,
      };
    });
