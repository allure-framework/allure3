...
for (const mode of reportModes) {
  test.describe(`Core Smoke (${mode})`, () => {
    test("loads stock navigation and resolves deep links", async ({ page }) => {
      ...
      await expect(page).toHaveTitle(uiDemo.widgets.summary);
      await expect(page.locator(".side-nav__menu .side-nav__text")).toHaveText(uiDemo.topLevelTabs);
    ...
    test("shows a loader and inline error when the tree test-result pane fails to load", async ({ page }) => {
      ...
      await expect(page.locator(".side-by-side__right .error-splash__message")).toHaveText("Not Found");
    });
  });
}
