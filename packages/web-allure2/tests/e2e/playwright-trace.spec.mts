...
test.describe("Playwright Trace", () => {
  for (const mode of [REPORT_MODES.SINGLE_FILE, REPORT_MODES.DIRECTORY] as const) {
    test(`opens the trace viewer and hands the trace over via postMessage (${mode})`, async ({ page }) => {
  ...
  test("loads single-file traces under the allure serve content security policy", async ({ page }) => {
    await mockTraceViewer(page);
    ...
    await page.route(`**/${playwrightTrace.name}/${REPORT_MODES.SINGLE_FILE}/index.html`, async (route) => {
    ...
    const leaf = page.locator(".node__leaf:visible").filter({ hasText: playwrightTrace.caseName }).first();
  ...
  test("does not apply the generic preview size limit to trace attachments", async ({ page }) => {
    ...
    await traceRow.click();
    await expect(page).toHaveURL(/attachment=/);
    await expect(previewContainerFor(traceRow).locator(".attachment-preview__preview-message")).toHaveCount(0);
