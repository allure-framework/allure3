...
test.describe("Dashboards", () => {
  test("single-file overview and graphs expose the stock widgets", async ({ page }) => {
    ...
    await expect(page.locator(".summary-widget__stats .splash__title")).toHaveText(uiDemo.totalTestCases);
    await expect(page.locator(".widget__title", { hasText: uiDemo.widgets.summary })).toBeVisible();
    await expect(page.locator(".widget__title", { hasText: uiDemo.widgets.trend })).toBeVisible();
    await expect(page.locator(".widget__title", { hasText: uiDemo.widgets.behaviors })).toBeVisible();
    await expect(page.locator(".widget__title", { hasText: uiDemo.widgets.suites })).toBeVisible();
    await expect(page.locator(".widget__title", { hasText: uiDemo.widgets.categories })).toBeVisible();
    await expect(page.locator(".widget__title", { hasText: uiDemo.widgets.environment })).toBeVisible();
    await expect(page.locator(".widget__title", { hasText: uiDemo.widgets.executors })).toBeVisible();
  ...
  test("directory mode loads overview and graphs through the external loader path", async ({ page }) => {
    await openReport(page, {
      fixture: uiDemo.name,
  ...
  test("links only whole absolute environment values", async ({ page }) => {
    ...
    await expect(absoluteUrl.getByRole("link", { name: "https://demo.allurereport.org" })).toHaveAttribute(
      "href",
      "https://demo.allurereport.org",
    );
    ...
    await expect(
      environmentWidget.locator(".table__row", { hasText: "demo.relative.url./index.html#/graphs" }).getByRole("link"),
    ).toHaveCount(0);
    await expect(
      environmentWidget
    ...
    await expect(
      environmentWidget.locator(".table__row", { hasText: "demo.plain.valueAPI - Local" }).getByRole("link"),
    ).toHaveCount(0);
    await expect(
      environmentWidget
    ...
    await expect(
      environmentWidget.locator(".table__row", { hasText: "demo.dangerous.urljavascript:alert(1)" }).getByRole("link"),
    ).toHaveCount(0);
  });
  ...
  test("validates explicit dashboard URL fields", async ({ page }) => {
    ...
    await expect(trendChart.getByRole("link", { name: "Open report for #12", exact: true })).toHaveAttribute(
      "href",
      "https://ci.qameta.io/job/allure2/job/master/Demo2_Report/index.html",
    );
    await expect(trendChart.getByRole("link", { name: "Open report for #11", exact: true })).toHaveCount(0);
  });
  ...
  test("directory mode keeps chart tooltips and resize behavior after graph route changes", async ({ page }) => {
    await openReport(page, {
      fixture: uiDemo.name,
      mode: REPORT_MODES.DIRECTORY,
    });
  ...
  test("keeps sibling widgets visible when one widget load fails", async ({ page }) => {
    ...
    await gate.seen;
    await expect(page.locator('.widget[data-id="executors"] .loader__text')).toHaveText("Loading...");
    await expect(page.locator('.widget[data-id="summary"] .summary-widget__stats')).toBeVisible();

    gate.release();

    await expect(page.locator('.widget[data-id="executors"] .error-splash__title')).toHaveText("500");
    await expect(page.locator('.widget[data-id="executors"] .error-splash__message')).toContainText(
