...
test.describe("Generic Attachments", () => {
  for (const mode of [REPORT_MODES.SINGLE_FILE, REPORT_MODES.DIRECTORY] as const) {
    test(`renders sandboxed interactive HTML attachment previews (${mode})`, async ({ page }) => {
      ...
      await expect(
        page.frameLocator(".attachment-row__preview .attachment-preview__frame").locator("[data-preview-ready='true']"),
      ).toBeVisible();

      await expect
        .poll(() => iframe.evaluate((node) => Number((node as HTMLElement).dataset.htmlPreviewHeight || 0)))
        .toBeGreaterThan(900);
      await expect.poll(() => iframe.evaluate((node) => node.getBoundingClientRect().height)).toBeLessThanOrEqual(1600);
    });
  }

  test("shows source and download fallback for invalid HTML attachment previews", async ({ page }) => {
    await openCaseFromTree(page, {
      fixture: attachmentsFixture.name,
  ...
  test("does not fetch oversized attachment previews when metadata exceeds the size cap", async ({ page }) => {
  ...
  test("renders media and URI-list attachments", async ({ page }) => {
    ...
    await expect(uriPreview.getByRole("link", { name: attachmentsFixture.uriLinks.docs })).toHaveAttribute(
      "href",
      attachmentsFixture.uriLinks.docs,
    );
  });
  ...
  for (const mode of [REPORT_MODES.SINGLE_FILE, REPORT_MODES.DIRECTORY] as const) {
    test(`renders the HTTP Exchange attachment with the HTTP exchange viewer (${mode})`, async ({ page }) => {
      await openCaseFromTree(page, {
        fixture: attachmentsFixture.name,
        mode,
        tab: "suites",
      ...
      await expect(requestSection.locator(".http-attachment__label", { hasText: /^Method$/ })).toHaveCount(0);
      await expect(requestSection.locator(".http-attachment__label", { hasText: /^URL$/ })).toHaveCount(0);
      ...
      await expect(requestSection.locator(".http-attachment__body-content .attachment-preview__code")).toContainText(
        '{"name":"demo","quantity":1}',
      );
      await expect(requestSection.locator(".http-attachment__body-content .attachment-preview__code")).toHaveClass(
        /language-json/,
      );
      ...
      await expect(requestDownload).toHaveAttribute("href", /^data:application\/json;charset=utf-8,/);
      ...
      await expect(responseSection.locator(".http-attachment__body-content .attachment-preview__code")).toContainText(
        "<script>window.__httpAttachmentXss = true</script>",
      );
      await expect(responseSection.locator(".http-attachment__body-content .attachment-preview__code")).toHaveClass(
        /language-json/,
      );
      ...
      await expect(aliasPreview.locator(".http-attachment__url")).toHaveText("https://api.example.com/v1/mime-alias");
      await expect(aliasPreview.locator(".http-attachment__status")).toHaveText("204 No Content");
    });

    test(`renders rich HTTP Exchange response bodies with attachment viewers (${mode})`, async ({ page }) => {
      await openCaseFromTree(page, {
        fixture: attachmentsFixture.name,
        mode,
        tab: "suites",
      ...
      const richImage = responseSection.locator(".http-attachment__body-content .attachment-preview__media");
    ...
    test(`shows no-view placeholder for unsupported HTTP Exchange bodies (${mode})`, async ({ page }) => {
      await openCaseFromTree(page, {
        fixture: attachmentsFixture.name,
        mode,
        tab: "suites",
      ...
      await expect(responseDownload).toHaveAttribute("href", /^data:application\/octet-stream;base64,/);
    });
    ...
    test(`renders form HTTP Exchange bodies (${mode})`, async ({ page }) => {
      ...
      await expect(preview.locator(".http-attachment__method")).toHaveText("POST");
      await expect(preview.locator(".http-attachment__url")).toHaveText("https://api.example.com/login");
    ...
    test(`renders multipart HTTP Exchange bodies (${mode})`, async ({ page }) => {
      ...
      await expect(httpMultipartRow).toHaveAttribute("data-type", "application/vnd.allure.http+json");
      await httpMultipartRow.click();

      const preview = previewContainerFor(httpMultipartRow);
      await expect(preview.locator(".http-attachment__method")).toHaveText("POST");
      await expect(preview.locator(".http-attachment__url")).toHaveText("https://api.example.com/profile");
      await expect(preview.locator(".http-attachment__status")).toHaveText("200 OK");
    ...
    test(`renders streaming HTTP Exchange bodies (${mode})`, async ({ page }) => {
      ...
      await expect(preview.locator(".http-attachment__method")).toHaveText("GET");
      await expect(preview.locator(".http-attachment__url")).toHaveText("https://api.example.com/events");
      await expect(preview.locator(".http-attachment__status")).toHaveText("200 OK");
      ...
      await expect(responseSection.locator(".http-attachment__body-content .attachment-preview__text")).toContainText(
        "event: ready",
      );
    });
  }

  test("renders a report-local download fallback for unsupported attachment types", async ({ page }) => {
    await openCaseFromTree(page, {
      fixture: attachmentsFixture.name,
      mode: REPORT_MODES.DIRECTORY,
      tab: "suites",
  ...
  test("keeps the tree layout when switching tabs after expanding an inline attachment", async ({ page }) => {
    await openCaseFromTree(page, {
      fixture: attachmentsFixture.name,
      mode: REPORT_MODES.DIRECTORY,
      tab: "suites",
    ...
    await expect(page.locator(".app__content .side-by-side__left allure-tree-view .tree__content")).toBeVisible();
    await expect(page.locator(".app__content .side-by-side__right .empty-view__message")).toContainText(
      "No item selected",
    );
  });
