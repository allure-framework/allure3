...
test.describe("Timeline", () => {
  test("filters visible results by duration and links bars to test results", async ({ page }) => {
    ...
    await expect(page.locator(".timeline__group_title").filter({ hasText: "ops-runner-eu-1" })).toHaveCount(1);
  ...
  test("renders the timeline brush controls for adjusting the visible window", async ({ page }) => {
    await openReport(page, {
      fixture: uiDemo.name,
      mode: REPORT_MODES.DIRECTORY,
      route: "timeline",
  ...
  test("renders retry results as status-colored hatched bars", async ({ page }) => {
    ...
    for (let index = 0; index < retryCount; index += 1) {
      ...
      const status = className.match(/\btimeline__item_status_(failed|broken|passed|skipped|unknown)\b/u)?.[1];
  ...
  test("renders zero-duration results as status-colored dots", async ({ page }) => {
    ...
    await expect.poll(async () => Number.isFinite(Number(await zeroDot.getAttribute("cx")))).toBe(true);
    ...
    await expect
      .poll(async () =>
        zeroResultLink.evaluate((node) => {
          ...
          return siblings.slice(dotIndex + 1).every((sibling) => !sibling.querySelector("rect.timeline__item"));
        }),
      )
      .toBe(true);
