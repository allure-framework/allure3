  ...
  text.replace(ansiControlSequencePattern, (match, sequence: string) => {
    ...
    for (let index = 0; index < codes.length; ) {
      ...
      if (
        ...
        flushDisplayCodes();
        fragments.push(`${ansiEscape}[48;2;${codes[index + 2]};${codes[index + 3]};${codes[index + 4]}m`);
        index += 5;
        continue;
      }
...
const ansi = (input: unknown): string =>
  ansiConverter.toHtml(
    normalizeAnsiForegroundColors(normalizeEscapedAnsiSequences(input == null ? "" : String(input))),
  );
