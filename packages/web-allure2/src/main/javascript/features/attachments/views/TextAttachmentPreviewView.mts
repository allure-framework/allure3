...
const renderLoadedTextAttachmentPreviewView = ({ fullScreen, previewData }: AttachmentPreviewOptions) => {
  const textContainer = createDiv(b("attachment-preview", { fullscreen: fullScreen, text: true }));
  textContainer.appendChild(
    createAnsiPre(b("attachment-preview", "text"), typeof previewData === "string" ? previewData : ""),
  );
