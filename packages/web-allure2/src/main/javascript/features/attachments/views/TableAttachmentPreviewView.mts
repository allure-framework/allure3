...
const parseTablePreviewData = (attachment: Attachment, previewData: unknown) => {
  ...
  return contentType === "text/tab-separated-values" ? tsvParseRows(content) : csvParseRows(content);
};

const renderLoadedTableAttachmentPreviewView = ({ attachment, fullScreen, previewData }: AttachmentPreviewOptions) => {
  const tableContainer = createDiv(b("attachment-preview", { fullscreen: fullScreen, table: true }));
