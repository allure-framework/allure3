...
type LoadedPreviewViewOptions<TData> = {
  ...
  toLoadedOptions: (data: TData, setCleanup: (cleanup?: () => void) => void) => Partial<AttachmentPreviewOptions>;
};
...
export const createPre = (className: string, text: unknown) => createElement("pre", { className, text: toText(text) });

export const joinClassNames = (...classNames: (string | null | undefined | false)[]) =>
  classNames.filter(Boolean).join(" ");

const attachmentUrl = (attachment: Attachment) => attachmentSourceUrl(attachment.source);
...
export const appendLinkSafely = (container: HTMLElement, text: string) => {
  const safeHref = sanitizeDetectedNavigationUrl(text);
  if (!safeHref) {
...
export const createAsyncAttachmentPreview = <TData,>({ createSuccess, load }: AsyncAttachmentPreviewOptions<TData>) => {
  ...
  Object.assign(el, {
    render() {
      ...
      void mountAsyncView({
        createSuccess: (data) => createSuccess(data, (cleanup) => (pendingCleanup = cleanup || null)),
        load,
        mount: mountSubView,
