...
const HtmlFrameAttachmentPreviewView = ({ attachment, fullScreen, previewData }: AttachmentPreviewOptions) => {
  const htmlContent = typeof previewData === "string" ? previewData : "";
  const htmlPreviewToken = `${attachment.uid || attachment.source}-${Math.random().toString(36).slice(2)}`;
  ...
  const onMessage = (event: MessageEvent) => {
    ...
    if (!data || data.type !== HTML_PREVIEW_RESIZE_MESSAGE_TYPE || data.token !== htmlPreviewToken) {
      return;
    }
...
const HtmlFallbackAttachmentPreviewView = ({
  ...
  if (htmlContent && getHtmlPreviewByteLength(htmlContent) <= INVALID_HTML_PREVIEW_SOURCE_MAX_BYTES) {
    htmlFallbackContainer.appendChild(
      createPre(`${b("attachment-preview", "code")} ${b("attachment-preview", "html-source")}`, htmlContent),
    );
  }
