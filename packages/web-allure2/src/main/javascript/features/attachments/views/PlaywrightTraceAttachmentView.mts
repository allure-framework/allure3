...
const PlaywrightTraceAttachmentContentView = (options: PlaywrightTraceAttachmentContentOptions) => {
  ...
  const postTraceToViewer = (iframe: HTMLIFrameElement) => {
    ...
    iframe.contentWindow?.postMessage({ method: "load", params: { trace: traceBlob } }, TRACE_VIEWER_ORIGIN);
  };

  const scheduleTraceHandoff = (iframe: HTMLIFrameElement) => {
    clearHandoffTimers();
    handoffTimers = TRACE_HANDOFF_DELAYS_MS.map((delay) => window.setTimeout(() => postTraceToViewer(iframe), delay));
  };
