...
class WidgetsGridElement extends BaseElement {
  ...
  getWidgetsArrangement() {
    ...
    const storedWidgets = savedData.map((col) => col.filter((widgetName) => this.widgets[widgetName]));
  ...
  saveWidgetsArrangement() {
    this.settings.setWidgetsArrangement(
      Array.from(this.querySelectorAll(".widgets-grid__col"))
        .map((colEl) =>
          Array.from(colEl.querySelectorAll(".widget")).map((element) => (element as HTMLElement).dataset.id),
        )
        .map((col) => col.filter((widgetName): widgetName is string => typeof widgetName === "string")),
    );
  }
