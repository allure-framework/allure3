...
const DurationTrendWidgetView = (options: DurationTrendWidgetOptions = {}) => {
  ...
  const mountChart = () => {
    ...
    const trendDirections: Array<(delta: number) => boolean> = [(delta) => delta >= 0, (delta) => delta <= 0];
    const lastExtremum = Math.min(
      ...trendDirections.map((op) =>
        values.reduce((memo, current, index) => (op(current - (values[index - 1] ?? 0)) ? index : memo), lastValue),
      ),
    );
    const amplitude = Math.max(...values) - Math.min(...values);
    const lastDelta = (values[lastExtremum] ?? 0) - lastValue;
    const level = scaleLinear<string>().domain([0, Math.max(amplitude, 0.25 * Math.max(...values))]);
