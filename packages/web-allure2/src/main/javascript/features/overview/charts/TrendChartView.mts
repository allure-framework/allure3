...
class TrendChartView extends BaseChartView {
  ...
  constructor(options: TrendChartOptions = {}) {
    ...
    this.keys = options.keys || Array.from(new Set(this.items.flatMap((item) => Object.keys(item.data || {}))));
  ...
  showSlices(data: TrendPoint[]) {
    ...
    edges
      ...
      .attr("width", (d, i) => (i === 0 || (this.x(d.id) ?? 0) === this.width ? this.x.step() / 2 : this.x.step()))
