...
export default class SeverityChartView extends BaseChartView {
  ...
  getChartData() {
    this.data = severities.map((severity) =>
      statuses.map((status): SeverityChartDatum => {
        const testResults = this.items.filter((item) => item.status === status && item.severity === severity);
        return {
  ...
  drawChart() {
    ...
    animatedBars.attr("y", (d) => this.y(d.value)).attr("height", (d) => this.height - this.y(d.value));
  }
