...
const byName = (a: TreeNode, b: TreeNode) => (String(a.name).toLowerCase() < String(b.name).toLowerCase() ? -1 : 1);
...
function compare(a: TreeNode, b: TreeNode, nodeCmp: TreeSorter, groupCmp: TreeSorter, direction: number) {
...
export default function getComparator({ sorter, ascending }: Pick<TreeSorting, "sorter" | "ascending">) {
