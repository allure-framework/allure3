...
const projectTreeChildren = (children: TreeNode[] = [], filter: TreeFilter, sorter: TreeSorter): TreeNode[] =>
  children
    .map((child) => {
      if (!child.children) {
        return child;
...
export const projectTreeData = (treeData: LoadedTreeData, filter: TreeFilter, sorter: TreeSorter): LoadedTreeData => {
