...
const createTreeTime = ({ value, name, tooltip }: { value?: number; name: string; tooltip: string }) =>
  isDef(value)
    ? createElement("span", {
...
const createTreeLeaf = (item: TreeRenderItem, { baseUrl, testResultTab }: TreeGroupRenderOptions) => {
...
const createTreeNodeElement = (item: TreeRenderItem, options: TreeGroupRenderOptions): HTMLElement => {
  if (!item.children) {
...
export const createTreeGroupChildren = (item: TreeRenderItem, options: TreeGroupRenderOptions): HTMLElement =>
  createElement("div", {
