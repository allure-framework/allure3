...
type TreeSettings = ReturnType<typeof import("../../../core/services/settings.mts").getSettingsForTreePlugin>;
...
class TreeViewElement extends BaseElement {
  ...
  getGroupElement(uid: string) {
    return this.querySelector(`.node[data-node-kind='group'][data-node-uid='${uid}']`) as HTMLElement | null;
  }
  ...
  findElement(treeNode: { testResult?: string; testGroup?: string }) {
    if (treeNode.testResult) {
      return this.querySelector(`[data-uid='${treeNode.testResult}'][data-parentUid='${treeNode.testGroup}']`);
    }

    return this.querySelector(`.node[data-node-kind='group'][data-node-uid='${treeNode.testGroup}'] > .node__title`);
  }
  ...
  onNodeClick(e: Event) {
    ...
    if (current?.testGroup && (current.testResult || current.testGroup !== uid) && this.isCurrentNodeInsideGroup(uid)) {
