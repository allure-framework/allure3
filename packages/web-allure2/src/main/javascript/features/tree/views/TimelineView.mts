...
class TimelineView extends BaseChartView {
  ...
  doShow() {
    ...
    const domain = [this.treeData.time.start || 0, this.treeData.time.stop || 0] as [number, number];
    ...
    this.xBrushAxis = this.makeAxis(
      ...
      {
        scale: this.chartX as unknown as import("d3-axis").AxisScale<import("d3-axis").AxisDomain>,
        tickFormat: (d: import("d3-scale").NumberValue) => duration(Number(d) - (this.treeData.time.start || 0), 2),
        tickSizeOuter: 0,
      },
      {
  ...
  drawTestResults(
    ...
    if (items.length) {
      ...
      const bars = barLinks
        ...
        .attr("width", (d: TimelineTreeNode) => this.chartX((d.time?.start || 0) + (d.time?.duration || 0)))
        .attr("rx", 2)
        .attr("ry", 2)
  ...
  onBrushChange(event: import("d3-brush").D3BrushEvent<unknown>) {
    ...
    const start = (d: TimelineTreeNode) => Math.max(0, Math.min(this.chartX(d.time?.start || 0), this.width));
    const stop = (d: TimelineTreeNode) => Math.max(0, Math.min(this.chartX(d.time?.stop || 0), this.width));
    if (selection) {
