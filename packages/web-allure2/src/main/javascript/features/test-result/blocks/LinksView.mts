...
const LinksView = (options: TestResultBlockOptions) => {
  ...
  const firstTruthy = (...values: Array<string | null | undefined>) => values.find((value) => value) || null;
  const renderLinks = (links: Array<{ type?: string; url?: string; name?: string }> | null | undefined) => {
