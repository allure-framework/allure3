...
const createLoadedTestResultData = (result: TestResult): LoadedTestResultData => {
  const allAttachments = makeArray<Stage>(result.beforeStages)
    ...
    .reduce<Attachment[]>((attachments, stage) => attachments.concat(collectAttachments(stage)), []);

  return {
    result,
    allAttachments,
    attachmentsByUid: allAttachments.reduce<Record<string, Attachment>>((attachments, attachment) => {
