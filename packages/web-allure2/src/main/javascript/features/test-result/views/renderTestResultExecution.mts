...
const createStepStats = ({
  ...
  if (parameters?.length) {
    stats.push(translate("testResult.stats.count.parameters", { hash: { count: parameters.length } }));
  }
  if (stepsCount) {
    stats.push(translate("testResult.stats.count.steps", { hash: { count: stepsCount } }));
  }
  if (attachmentsCount) {
    stats.push(translate("testResult.stats.count.attachments", { hash: { count: attachmentsCount } }));
  }
...
const createStagesBlock = ({ stages, name, expanded }: { stages?: Stage[]; name: string; expanded: boolean }) =>
  stages?.length
    ? createElement("div", {
...
export const createTestResultExecutionContent = ({ hasContent, before, test, after }: ExecutionTemplateOptions) =>
  createFragment(
    createElement("h3", {
