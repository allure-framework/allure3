...
import { appendMountable, attachMountable, destroyMountable } from "../../../core/view/mountables.mts";
    ...
    render() {
      ...
      el.replaceChildren(
        createTestResultOverviewContent({
          ...
          statusMessage: typeof options.data.statusMessage === "string" ? options.data.statusMessage : undefined,
          statusTrace: typeof options.data.statusTrace === "string" ? options.data.statusTrace : undefined,
        }),
      );
      el.showBlock(el.querySelector(".test-result-overview__tags"), getTestResultBlocks("tag"));
      el.showBlock(el.querySelector(".test-result-overview__before"), getTestResultBlocks("before"));
