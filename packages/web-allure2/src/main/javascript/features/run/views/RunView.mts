        ...
        ? createElement("dl", {
            ...
            children: [
              ...
              typeof error.actual === "string"
                ? [createElement("dt", { text: translate("run.actual") }), createElement("dd", { text: error.actual })]
                : null,
            ],
          })
        : null,
...
class RunViewElement extends BaseElement {
  ...
  onAttachmentClick(event: Event) {
    ...
    if (target instanceof Element && target.closest("[data-download], .attachment-row__fullscreen")) {
      return;
    }
    ...
    if (row.classList.contains("attachment-row_selected") && this.getMountedChild(name)) {
      ...
      this.mountChild(name, AttachmentView({ attachment: { ...attachment, uid } }), this.querySelector(`.${name}`));
    }
  ...
  onAttachmentFullscreenClick(event: Event) {
    ...
    const uid = (event.currentTarget as Element).closest(".attachment-row")?.getAttribute("data-uid");
