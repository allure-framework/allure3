...
export class ReportDataError extends Error {
  ...
  constructor({ cause, message, status = DEFAULT_REPORT_ERROR_STATUS, url = "" }: ReportDataErrorOptions) {
    super(message);
...
export const reportDataUrl = async (url: string, contentType = "application/octet-stream"): Promise<string> => {
...
const fetchReportData = async (
  ...
  const fetchUrl = url.startsWith("data:") || url.startsWith("blob:") ? url : await reportDataUrl(url, contentType);
...
export const fetchReportJson = async <TPayload,>(url: string, options: ReportFetchOptions = {}): Promise<TPayload> => {
...
export const fetchReportText = async (url: string, options: ReportFetchOptions = {}): Promise<string> => {
  const response = await fetchReportData(url, options);

  return response.text();
};

export const fetchReportBlob = async (url: string, options: ReportFetchOptions = {}): Promise<Blob> => {
  const response = await fetchReportData(url, options);
