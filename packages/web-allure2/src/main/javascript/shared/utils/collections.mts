...
export const uniq = <TItem,>(items: ReadonlyArray<TItem> | null | undefined) => Array.from(new Set(items || []));
...
) => (items || []).find((item) => Object.entries(attrs).every(([key, value]) => item && item[key] === value));
