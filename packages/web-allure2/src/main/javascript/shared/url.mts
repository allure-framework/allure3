...
const normalizeCandidate = (value: string) => stripIgnoredSchemeCharacters(decodeHtmlEntities(value.trim()));
...
const sanitizeNormalizedNavigationUrl = (rawValue: string, normalizedValue: string): string | null => {
  if (!normalizedValue) {
    return null;
  }
