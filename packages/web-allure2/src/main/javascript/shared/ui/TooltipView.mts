...
class TooltipView {
  ...
  private resolveClassName(position = this.activePosition) {
    ...
    const positionClassBase = this.positionClassBase || baseClassName.split(/\s+/).pop() || "tooltip";
  ...
  protected render() {
    ...
    if (typeof this.content === "string") {
      this.el.replaceChildren(...(this.content ? [createFragmentFromHtml(this.content, this.el)] : []));
  ...
  private getPositionCoordinates(anchorRect: TooltipRect, tooltipRect: TooltipSize, position: TooltipPosition) {
  ...
  private clampToViewport({ left, top }: TooltipViewportPosition, tooltipRect: TooltipSize): TooltipViewportPosition {
    const viewportRect = this.getViewportRect();
  ...
  private resolvePosition(anchorRect: TooltipRect, tooltipRect: TooltipSize, position: TooltipPosition) {
  ...
  show(text: TooltipContent, anchor: Element) {
    const position = this.resolvePreferredPosition((this.options.position || "top") as TooltipPosition);
