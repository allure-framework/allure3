...
describe("generateTestResults", () => {
  ...
  it.each([
    {
      caseName: "a storage run root",
      url: "https://storage.example/run-123",
      expected: `https://storage.example/run-123/awesome#${historicalResultId}`,
    },
    {
      caseName: "the existing query and fragment behavior",
      url: "https://storage.example:8443/reports/run-123?ignored=1#old-fragment",
      expected: `https://storage.example:8443/reports/run-123/awesome#${historicalResultId}`,
    },
    {
      caseName: "the existing trailing-slash behavior",
      url: "https://storage.example/run-123/",
      expected: `https://storage.example/run-123//awesome#${historicalResultId}`,
    },
    {
      caseName: "an empty history URL",
      url: "",
      expected: "",
    },
  ])("should resolve $caseName into a copied history item", async ({ url, expected }) => {
    const item: HistoryTestResult = Object.freeze({
      id: historicalResultId,
      historyId: "stable-history-id",
    ...
    const sourceHistory = [item];
    const { writer, store, testResult } = historyFixture(sourceHistory);

    const [converted] = await generateTestResults(writer, store, [testResult], { pluginId: "awesome" });

    expect(converted.history).toStrictEqual([{ ...item, url: expected }]);
    expect(converted.history).not.toBe(sourceHistory);
    expect(converted.history[0]).not.toBe(item);
    expect(item.url).toBe(url);
    expect(sourceHistory).toStrictEqual([item]);
  });
