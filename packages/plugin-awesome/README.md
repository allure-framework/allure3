...
| `groupBy`        | Grouping tests by labels or combining labels and `titlePath` at the end. By default, tests are grouped using the `titlePath` provided by the test framework. | `string` `string[]`                                          | Grouping by `titlepath`                                    |
| `__titlePath`    | Special marker for `groupBy`. When placed as the last item in the array, forces a final grouping by `titlePath` after all label-based groups. | _internal marker_ | Must be last, i.e `["package", "language", "__titlePath"]` |
